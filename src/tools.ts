...
export const TOOL_DEFINITIONS = [
  ...
  { type: 'function' as const, function: { name: 'workflow', description: 'Run a dynamic workflow: a small deterministic JavaScript script that orchestrates many isolated sub-agents (parallel/pipeline fan-out, phases, structured output) and synthesizes their results. Use for codebase audits, multi-dimension review, fan-out research, or large refactors where the control flow (loops, conditionals, fan-out) should be explicit code rather than improvised turn by turn. The script must begin with `export const meta = { name, description, phases }` and use the globals agent()/parallel()/pipeline()/phase()/log()/args/budget. Returns the script result plus a progress summary.', parameters: { type: 'object', properties: { script: { type: 'string', description: 'The workflow JavaScript. First statement must be `export const meta = {...}` (a pure literal). Then call agent(prompt, opts)/parallel(thunks)/pipeline(items, ...stages)/phase(title) and `return` the final result. Determinism: Date.now()/Math.random()/new Date()/require/import/fs are unavailable.' }, args: { description: 'Optional JSON value exposed to the script as the global `args`.' } }, required: ['script'] } } },
  // ── Task Management ─────────────────────────────────
