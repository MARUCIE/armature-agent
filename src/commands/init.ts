/**
 * `armature init` — Initialize Armature configuration.
 *
 * Creates:
...
export function createInitCommand(): Command {
  return new Command('init')
    .description('Initialize armature configuration for this project')
    ...
    .action(async (opts: { globalOnly?: boolean }) => {
      ...
      printInfo('     armature chat                      # interactive REPL (42 tools)')
      printInfo('     armature chat "explain this code"  # one-shot query')
      printInfo('')
      printInfo('  3. Run an agent task:')
      printInfo('     armature run "fix failing tests"   # task execution mode')
      printInfo('')
