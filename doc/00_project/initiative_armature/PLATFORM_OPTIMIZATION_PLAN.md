---
Title: Armature CLI Platform Optimization Plan
Scope: project optimization plan
Owner: Maurice
Status: Active
LastUpdated: 2026-05-29
---
# Armature CLI Platform Optimization Plan

## 2026-05-29 Optimization Delta - Multi-Model Review Ledger

The large-PR review workflow is now integrated as an Armature CLI command:

1. Multi-model review pod:
   - `armature review-ledger` runs independent model review prompts for the same PR/diff.
...
5. Verification:
   ...
   - dist-level dry-run smoke wrote `10` review artifacts under `/tmp/armature-review-ledger-smoke`
...
## 2026-05-02 Optimization Delta - Tool-Call Closure and Armature Mark
...
3. Blackfin startup cleanup:
   - replaced the compact `ORCA` startup mark with a dominant `ARMATURE-AGENT` wordmark
...
4. Verification:
   ...
   - post-screenshot Banner render inspection shows `ARMATURE-AGENT`, `Armature Agent v0.8.16`, `Blackfin Signal`, clean state rows, and no independent hero art
...
1. Terminal copyability:
   ...
   - added Claude-style no-flicker aliases: `ARMATURE_TUI=fullscreen`, `ARMATURE_NO_FLICKER=1`, and `CLAUDE_CODE_NO_FLICKER=1`
...
2. Workspace reliability:
   ...
   - added last-workspace persistence under `~/.armature/last-cwd`
   - forwarded root `armature --cwd <dir>` into chat for launcher/menu entrypoints
3. Tool system:
...
2. Command surface:
   - added `armature critique` as a first-class command
   - supports plan/log/diff inputs, risk flags, `--force`, `--dry-run`, `--json`, and `--show-prompt`
   - added `/critique` inside `armature chat` for local read-only risk inspection without a model call
...
3. Automatic chat hint:
   ...
   - extended the notice to one-shot `armature chat "prompt"` streaming runs while leaving `--json` output clean
   - kept automatic hints model-free, prompt-preserving, and configurable through `--no-auto-critique`, `--auto-critique-threshold`, `ARMATURE_AUTO_CRITIQUE`, and `ARMATURE_AUTO_CRITIQUE_THRESHOLD`
4. Reviewer routing:
...
6. Reliability cleanup:
   - fixed project hook trust evaluation so `ARMATURE_TRUST_PROJECT_HOOKS=1` is honored at hook load time even when the singleton already exists
...
1. Status identity:
   - kept `ARMATURE POD`, model, context bar / percentage, and branch on the first status line
...
1. Transcript roles:
   ...
   - changed assistant response panels from `ORCA` to `ARMATURE POD`
...
2. Tool and thinking feedback:
   ...
   - replaced the broad generic thinking verb set with compact Armature/pod/proof-oriented verbs
3. Verification:
   - focused Ink UI tests, lint, build, release-evidence test, real CLI version smoke, scoped diff check, and full suite passed
...
1. Shared UI system:
   - made `PickerFrame.tsx` theme-aware so picker chrome uses Armature semantic tokens instead of a generic cyan default
...
1. Identity:
   - shipped `Blackfin Signal` as the Armature default dark theme
   - replaced generic startup art with an `ARMATURE-AGENT` block wordmark and clean startup deck
2. UI hierarchy:
...
2. Queue visibility:
   - ship `armature queue list/show/follow/takeover/evidence/resume/schedule`
...
4. Evidence console:
   - `armature queue evidence` now opens TaskRun logs, diffs, data, reports, missing artifacts, and capped previews
...
Keep Armature CLI maintainable as a fast-moving CLI runtime while preventing drift across docs, command surfaces, and provider-routing behavior.
...
| Project docs structure | Flat legacy docs plus new initiative tree | Use `doc/00_project/initiative_armature/` as canonical source going forward |
...
| Hermes-inspired runtime ergonomics | Armature lacked detached-job and oversized-result UX | Keep high-value runtime resilience features in Armature where no gateway abstraction is required |
...
| Provider inspection ergonomics | `armature providers` only showed a thin readiness table | Reuse the same model catalog so provider inspection and REPL selection stay consistent |
...
| Operator-wide hook ergonomics | Personal hooks required per-project `.armature/hooks.json` duplication | Load `~/.armature/hooks.json` as a global native hook source |
...
| Headless parity | `armature serve` originally exposed a thin status surface | Reuse doctor/model metadata in server endpoints instead of inventing a second observability model |
| Stats visibility | `armature stats` only covered usage/cost | Merge runtime health and error signals into the stats surface |
...
4. Expand headless/API documentation when `armature serve` grows beyond current HTTP + SSE scope.
...
- `doc/00_project/initiative_armature/SOTA_EXPERIENCE_GAP_REPORT.md`
...
- top-level `armature permissions` command for inspecting and persisting approval mode
...
- this gives Armature a first explicit workflow-packaging surface without introducing a second preset framework
...
What this means for Armature:

- Wave 3 is now relatively mature compared with the market on workflow packaging and trust clarity.
- Armature’s biggest remaining product gap is Wave 4 continuity, not another round of naming or picker polish.
...
- CLI resume now supports exact durable objects through `armature -c <id>`
- `armature serve` now exposes:
  - `GET /sessions`
  - `GET /sessions/latest`
  - `GET /sessions/:id`
- this establishes a headless continuity discovery layer before richer hosted/web take-over flows
- `armature run` now writes TaskRun records for default, goal-loop, mission, and plan branches, so queue inspection can see status, summary, usage, and mission-state evidence
- `armature chat` REPL now writes per-prompt TaskRun records under the active chat WorkSession, so queue inspection can see interactive turn status and usage
- `armature queue resume` now claims a resume lease and prints the saved-session continuation command for chat TaskRuns
- `armature queue schedule` now selects the next unleased resumable or monitorable TaskRun instead of treating leases as passive metadata
