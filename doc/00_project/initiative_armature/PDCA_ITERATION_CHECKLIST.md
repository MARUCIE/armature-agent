---
Title: Armature CLI PDCA Iteration Checklist
Scope: project PDCA checklist
Owner: Maurice
Status: Active
LastUpdated: 2026-05-29
---
# Armature CLI PDCA Iteration Checklist
...
| Gate | Result | Evidence |
| --- | --- | --- |
| Plan | Pass | Long-session local-file failure, canonical tool-call matrix closure, and Hermes-style Armature mark refresh scoped together |
| Do | Pass | `streamChat()` system prompt persistence, local-file prompt contract, runtime local-file intent guard, false-save repair, `test:tool-calls` matrix layer, generated entrypoints, and `Banner.tsx` `ARMATURE-AGENT` clean-deck icon removal |
| Check | Pass | provider/system-prompt pack -> `22`; local-file guard pack -> `31`; `npm run lint`; `npm run test:tool-calls`; focused Ink UI pack -> `80`; `npm test` -> `91` files / `1663` tests; dist tool smoke passed; rendered Banner inspection confirmed new `ARMATURE-AGENT` first frame |
...
| Check | Pass | targeted regression pack -> `250`; `npm run build`; root/chat help smoke; `armature doctor`; live Poe `read_file` smoke; `npm test` -> `91` files / `1663` tests |
...
## 2026-05-01 - Cute Armature Mascot UI/UX
...
| Do | Pass | `ARMATURE_VISUAL_SYSTEM_PLAN.md/html`, `Blackfin Signal`, `Blackfin Pod Deck`, `POD SIGNAL`, `ARMATURE POD` |
...
| Gate | Result | Evidence |
| --- | --- | --- |
| Plan | Pass | `ARMATURE_VISUAL_SYSTEM_PLAN.md`, `ARMATURE_VISUAL_SYSTEM_PLAN.html`, `task_plan.md` ORCA-VIS queue |
| Do | Pass | Killer-whale `Blackfin Signal` identity in `src/ui/theme.tsx`, `Banner.tsx`, `HomePanel.tsx`, `ThemePicker.tsx`, `StatusBar.tsx` |
| Check | Pass | `npm test -- tests/ink-ui.test.tsx` -> `77`; `npm run lint`; `npm run build`; `ARMATURE_THEME=armature node dist/bin/armature.js --version` -> `0.8.16`; `npm test` -> `88` files / `1625` tests |
