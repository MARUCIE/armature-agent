---
Title: Armature CLI PDCA Execution Plan
Scope: project PDCA execution plan
Owner: Maurice
Status: Active
LastUpdated: 2026-05-29
---
# Armature CLI PDCA Execution Plan
...
- Plan:
  - Fix Armature's Ink terminal behavior so default output is copyable and does not flicker through alternate-screen startup.
...
- Do:
  - Made `AlternateScreen` opt-in through `ARMATURE_ALT_SCREEN=1`.
  - Added Claude-style no-flicker aliases through `ARMATURE_TUI=fullscreen`, `ARMATURE_NO_FLICKER=1`, and `CLAUDE_CODE_NO_FLICKER=1`.
  - Moved no-flicker alternate-screen entry before Ink's first frame and capped rendered history in no-flicker mode.
  - Made mouse tracking opt-in through `ARMATURE_MOUSE=1`.
  - Added workspace cwd resolution and last-workspace memory in `chat-support`.
  - Forwarded root `armature --cwd <dir>` into `armature chat`.
...
- Check:
  ...
  - `node dist/bin/armature.js doctor` -> provider OK and `14` MCP configs discovered.
  - Live Poe one-shot tool smoke -> model used `read_file`; tool result succeeded; final answer `Armature CLI`.
  - `npm test` -> `91` files / `1663` tests passed.
- Act:
...
- Do:
  ...
  - Added `src/commands/critique.ts` for `armature critique` with dry-run, JSON, plan/log/diff inputs, and risk flags.
  ...
  - Exposed `armature chat --no-auto-critique` and `armature chat --auto-critique-threshold <score>` for session-scoped automatic hint control.
  - Extended the automatic local hint to one-shot `armature chat "prompt"` streaming runs while preserving clean `--json` output.
...
- Plan:
  - Extend the Armature killer-whale / pod identity into the persistent shortcut footer.
...
- Plan:
  - Extend the Armature killer-whale / pod intelligence identity into live multi-model progress.
...
- Plan:
  - Extend the Armature killer-whale / pod identity into detail panels that surface status, permission, notes, thread, and TaskRun evidence.
...
- Plan:
  - Extend the Armature killer-whale / pod identity into approval and write-review boundaries.
...
- Plan:
  - Extend the Armature killer-whale / pod identity into the compact post-turn summary.
...
- Plan:
  - Extend the Armature killer-whale / pod identity into the fixed StatusBar.
  - Keep the change bounded to status rendering, focused tests, and active docs.
- Do:
  - Added `sonar` context language to the first status line while preserving `ARMATURE POD`, model, context bar, percentage, and branch.
...
- Plan:
  - Extend the Armature killer-whale / pod identity into live transcript roles, tool rails, and thinking feedback.
  - Keep the change bounded to Ink rendering components, focused tests, and active docs.
- Do:
  - Changed user prompt blocks to `POD BRIEF`.
  - Changed assistant response panels to `ARMATURE POD` and streaming copy to `echoing`.
  - Added `ECHO TOOL` to active and completed tool rails.
  - Replaced broad generic thinking verbs with compact Armature / pod / proof-oriented verbs.
  - Updated focused Ink UI tests.
- Check:
...
- Plan:
  - Extend the Armature killer-whale / pod identity from the first frame into high-frequency operation surfaces.
...
- Do:
  ...
  - Updated `OptionPicker.tsx` to use Armature semantic tokens for selection, labels, descriptions, and scroll affordances.
...
## 2026-05-01 - Cute Armature Mascot UI/UX PDCA
...
- Plan:
  ...
  - Keep Armature original without copying Hermes caduceus and without rendering a separate startup mascot/icon.
- Do:
  - The former `ARMATURE_MASCOT_LINES` path was later removed from the startup Banner.
...
- Do:
  - Updated `ARMATURE_VISUAL_SYSTEM_PLAN.md` and `.html` with the explicit hierarchy.
  - Kept the shipped UI naming aligned to `Blackfin Signal`, `Blackfin Pod Deck`, `POD SIGNAL`, and `ARMATURE POD`.
- Check:
...
- Design source: `ARMATURE_VISUAL_SYSTEM_PLAN.md`
- Human companion: `ARMATURE_VISUAL_SYSTEM_PLAN.html`
- Plan source: `task_plan.md` ORCA-VIS queue
- Plan:
  - Adopt `Blackfin Signal` as Armature's terminal identity.
  - Learn Hermes's high-recognition mechanics without copying its caduceus or exact gold-only palette.
  - Anchor Armature's identity in the killer whale idea: blackfin, pod, dorsal-fin silhouette, and echolocation.
...
- Done tranche:
  ...
  - `armature queue list/show/follow/takeover/evidence`
  ...
  - `armature run` default / goal-loop / mission / plan WorkSession and TaskRun records
  ...
  - `armature queue resume` claims a resume lease and prints a concrete `armature chat --continue <saved-session-id>` command for resumable chat TaskRuns
  - `armature queue schedule` claims the next unleased resumable or monitorable TaskRun and prints the recovery action
- Check evidence:
  - `npm run lint`
  - `npm run build`
...
- Use `SOTA_EXPERIENCE_GAP_REPORT.md` as the canonical competitive-gap source before choosing the next Armature UX tranche.
...
- Fill `AGENT_EVAL_PLAN.md` with Armature-specific task / grader / evidence requirements.
