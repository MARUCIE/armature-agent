# Armature CLI Visual System Plan

## Purpose

This plan upgrades the local Armature CLI Ink interface into a recognizable terminal product identity. The target is the primary Armature surface, not a browser UI: startup banner, empty-state home panel, theme selection, status bar, transcript panels, and command affordances.

User request, 2026-04-30: optimize local Armature CLI, study Hermes Agent, create a distinctive visual system across typography, color, UI, and UX, design first, then execute through PDCA.

2026-05-02 correction: the startup deck must use a dominant `ARMATURE-AGENT` display wordmark and a clean Blackfin Signal state deck. The older compact `ORCA` lockup is no longer sufficient, and the later independent Armature icon/hero mark has been removed after operator feedback.
...
2. One memorable symbolic system: Hermes uses a caduceus; Armature should rely on the `ARMATURE-AGENT` wordmark, Blackfin Signal language, and restrained dorsal-fin / echolocation motifs instead of a startup mascot.
...
Concept: Armature as a killer-whale pod command surface. The interface should feel precise, predatory, and coordinated: blackfin / foam contrast, amber echolocation signal lights, and reef/kelp/coral accents for state and action.
...
3. Product metaphor: pod intelligence. Armature CLI should feel like coordinated operator work in a terminal, not like a generic ocean-themed UI.

Rule: ocean imagery supports Armature; it must not become the primary brand. Avoid generic "deep sea" language unless it is tied back to killer-whale motion, pod behavior, or echolocation.
...
The first screen should be identifiable from across the room: a block `ARMATURE-AGENT` display mark and a clean mission-control layout that makes trust, model, session, tools, and recovery paths visible before the user types.

## Typography

CLI cannot load fonts; the terminal owns the actual typeface. Armature should still define a typography system:

| Role | Implementation | Intent |
| --- | --- | --- |
| Display wordmark | Fixed block-letter `ARMATURE-AGENT` glyph lines | Brand identity, equivalent to a display font |
...
- Avoid proportional fonts and overly decorative patched fonts for the primary Armature session
...
- Armature adds blackfin / oceanic secondary colors so the interface does not become a one-note yellow theme.
...
This keeps the current UX principle of one primary action while making the page feel owned by Armature.

### Status Bar

Keep the 3-line shape, but tune the language:

- `ARMATURE POD` instead of plain `ORCA`
...
Carry the same Armature identity into the high-frequency operation surfaces:

- `PickerFrame` is theme-aware and uses the active Armature semantic border, title, subtitle, and footer tokens.
- `CommandPicker` is framed as `POD COMMANDS`, with `echo filter` language, compact key hints, and a visible no-match state.
- `OptionPicker` uses Armature tokens for selected rows, filter labels, descriptions, and scroll affordances.
...
This keeps slash commands and pickers ordinary and discoverable while making the surface unmistakably Armature.
...
- assistant response panels render as `ARMATURE POD`
...
- the original source title remains visible after the Armature frame label
...
| --- | --- | --- |
| Plan | Add this design plan, update task plan, rolling ledger, and active PDCA docs | `ARMATURE_VISUAL_SYSTEM_PLAN.md`, `ARMATURE_VISUAL_SYSTEM_PLAN.html` |
| Do | Tokenize the new theme and update Banner, HomePanel, ThemePicker, StatusBar copy | `src/ui/theme.tsx`, `src/ui/components/*` |
| Check | Run focused Ink tests, typecheck/build, then real CLI smoke with `ARMATURE_THEME=armature` | `tests/ink-ui.test.tsx`, `npm run build`, `node dist/bin/armature.js --version`, captured output |
| Act | Sync PRD, architecture, UX map, optimization plan, checklist, notes, deliverable, rolling ledger | active initiative docs |

## Acceptance Criteria

- Default dark theme resolves to `armature` / `Blackfin Signal`.
...
- Transcript blocks use `POD BRIEF`, `ARMATURE POD`, and `ECHO TOOL` while preserving content and status details.
