...
This audit compared Armature against current coding-agent SOTA patterns, then executed the first PDCA tranche against the highest-risk gaps.
...
Armature has moved beyond basic CLI parity. The strongest areas are provider-neutral execution, multi-model collaboration, workflow presets, saved sessions, and a growing continuity spine.
...
Before the fix, Armature loaded `.armature/hooks.json`, `.armature.json`, `.claude/hooks.json`, and `.claude/settings.json` from the current repository during startup. `SessionStart` hooks could run shell commands with inherited process environment.
...
- Project hooks now require explicit trust via `HookManager({ trustProjectHooks: true })` or `ARMATURE_TRUST_PROJECT_HOOKS=1`.
...
- Added `armature queue`.
- Added `armature queue list --status <status> --work-session <id> --limit <n>`.
- Added `armature queue show <task-run-id>`.
- Added `armature queue follow <task-run-id>` for live evidence tails and terminal-state exit.
- Added `armature queue takeover <task-run-id>` for explicit operator leases.
- Added `armature queue evidence <task-run-id>` as a TaskRun evidence drawer for logs, diffs, data, reports, missing artifacts, and capped previews.
...
- `armature queue evidence <task-run-id>` opens a terminal evidence drawer keyed by `TaskRun`.
...
- Added `doc/00_project/initiative_armature/verification_snapshot.json` as the active release evidence snapshot.
...
| M1 Queue Visibility | Make current TaskRun state inspectable | `armature queue list/show/follow/takeover/evidence` shipped; status filtering, evidence streaming, leases, and evidence drawer covered |
...
| M3 Evidence Console | Review-before-apply becomes inspectable | Done: CLI evidence drawer and Ink `/evidence` detail panel show TaskRun logs/diffs/artifacts plus approval timelines |
...
| M4e Scheduler / Resume Semantics | Queue leases become actionable recovery plans | `armature queue resume` and `armature queue schedule` claim leases and print chat continue or monitor commands |
...
| --- | --- | --- | --- | --- |
| ARMATURE-SWARM-001 | P0 | Require explicit trust before loading repo-local hooks | security | done |
| ARMATURE-SWARM-002 | P0 | Stop inheriting provider API keys into hook subprocess env | security | done |
| ARMATURE-SWARM-003 | P0 | Approval-gate `fetch_url` and `web_search` in auto mode | security | done |
| ARMATURE-SWARM-004 | P0 | Block private/loopback/link-local targets in `fetch_url` | security | done |
| ARMATURE-SWARM-005 | P1 | Add `armature queue list/show` over `TaskRun` records | runtime | done |
| ARMATURE-SWARM-006 | P1 | Add `queue follow` for live log/evidence streaming | runtime | done |
| ARMATURE-SWARM-007 | P1 | Add `queue takeover` lease model | architecture | done |
| ARMATURE-SWARM-008 | P1 | Convert `serve /chat` into a canonical run endpoint | architecture | done |
| ARMATURE-SWARM-009 | P1 | Add evidence drawer for TaskRun logs/diffs/artifacts | UX | done |
| ARMATURE-SWARM-010 | P1 | Centralize slash-command registry so HomePanel/completer/TUI cannot drift | UX | core done |
| ARMATURE-SWARM-011 | P2 | Align README/doc test counts to current suite evidence | docs | done |
| ARMATURE-SWARM-012 | P2 | Add CI matrix/security/performance/eval enforcement | verification | done |
| ARMATURE-SWARM-013 | P1 | Record `armature run` default/goal-loop/mission/plan executions as canonical TaskRuns | runtime | done |
| ARMATURE-SWARM-014 | P1 | Restore clean-index build for workflow, permissions, evolve, and git-root command surfaces | build | done |
| ARMATURE-SWARM-015 | P1 | Promote chat operator controls for sessions, permissions, models, command output, and Ink home/detail surfaces | UX | done |
| ARMATURE-SWARM-016 | P1 | Record chat REPL turns as canonical WorkSession / TaskRun records | runtime | done |
| ARMATURE-SWARM-017 | P1 | Expose TaskRun evidence in Ink DetailPanel through `/evidence` | UX | done |
| ARMATURE-SWARM-018 | P1 | Add review-before-apply approval timeline over TaskRun evidence | UX | done |
| ARMATURE-SWARM-019 | P1 | Make Ink submitted prompts visible and assistant markdown structurally rendered | UX | done |
| ARMATURE-SWARM-020 | P1 | Extend TaskRun leases into true scheduler / resume semantics | runtime | done |
| ARMATURE-SWARM-021 | P1 | Continue model-routing evidence and catalog SSoT | runtime | done |
...
- `armature queue evidence <task-run-id>` -> evidence drawer renders typed entries, metadata, missing files, and tail previews
- `/evidence <task-run-id>` -> Ink detail panel renders the shared TaskRun evidence drawer model
- Review-before-apply approvals -> prompted, preapproved, policy-blocked, and hook-blocked decisions append to `TaskRun.approvals` and render above file evidence
...
- `armature queue resume <chat-task-run> --holder <name>` -> claims a lease and prints the saved-session continue command
- `armature queue schedule --holder <name>` -> skips active leases and unsupported replay records, then claims the next resumable chat TaskRun
...
- `node dist/bin/armature.js queue list --limit 3` -> empty-state renders successfully
...
- Repo-local hooks now have an env trust switch, but a first-class `armature hooks trust` UX does not exist yet.
- `armature queue resume` supports chat saved-session continuation and running background-job monitoring; non-chat `run` replay remains unsupported until replay-safe argv/prompt metadata is persisted.
...
- HomePanel registry consumption remains blocked by the existing uncommitted UI baseline and should be split before claiming full ARMATURE-SWARM-010 closure.
