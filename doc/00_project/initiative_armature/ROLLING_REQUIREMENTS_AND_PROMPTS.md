...
| --- | --- | --- | --- | --- |
| REQ-20260503-010 | UX | Active Armature tasks must be interruptible with Claude-style Esc/Ctrl-C behavior | Done | `src/ui/keybindings.ts`, `src/ui/components/InputArea.tsx`, `src/providers/openai-compat.ts`, `tests/ink-ui.test.tsx` |
...
| --- | --- | --- | --- |
| PROMPT-20260503-010 | `armature 已开始的任务 无法按esc终止；学习下claude code 的快捷键体系，全部实现` | Claude Code official-doc parity audit + terminal UX implementation | Added keybinding metadata, abort propagation, and regression coverage |
...
| Should Armature create a Markdown file from a response that only says it saved the file? | No. Without extractable artifact content, repair must return `null`. | `tests/local-file-intent.test.ts` |
...
| ID | Type | Requirement | Status | Evidence |
| --- | --- | --- | --- | --- |
| REQ-20260502-024 | Reliability | Streamed provider turns must include the current Armature system prompt even when chat history exists | Done | `src/providers/openai-compat.ts`, `tests/openai-compat-multimodal.test.ts` |
...
| REQ-20260502-027 | UI | Startup identity must use a dominant `ARMATURE-AGENT` wordmark and clean Blackfin Signal deck with no independent mascot/icon/hero art | Done | `src/ui/components/Banner.tsx`, `tests/ink-ui.test.tsx` |
...
| REQ-20260502-031 | UI | The startup Banner must explicitly delete the separate Armature icon/hero block after operator rejection | Done | `src/ui/components/Banner.tsx`, `tests/ink-ui.test.tsx` |
...
| PROMPT-20260502-010 | `armature 的logo 还是老得，要学习hermes agent` | UI identity correction | Replaced the old compact `ORCA` wordmark with a dominant `ARMATURE-AGENT` wordmark; the later hero panel is superseded by `PROMPT-20260502-012` |
...
| Can Armature answer that it cannot create/open local files before trying tools? | No. It must use the relevant local file tool first and only report a concrete tool failure. | `src/system-prompt.ts`, `tests/e2e-workflow.test.ts` |
...
| If the user says a previously claimed file is missing and asks to open it, should Armature ask them to copy content manually? | No. The REPL guard reconstructs the claimed file from assistant history, writes it, and opens it when possible. | `tests/local-file-intent.test.ts`, `tests/chat-repl-turn.test.ts` |
...
| Should Armature copy Hermes' caduceus/skin assets? | No. It should copy the brand structure only and keep a distinct Armature Blackfin mark. | `src/ui/components/Banner.tsx`, `tests/ink-ui.test.tsx` |
| Should the startup Banner render a separate Armature icon, mascot, or hero block? | No. The rejected hero block is deleted; the first frame uses the `ARMATURE-AGENT` wordmark and clean state deck. | `tests/ink-ui.test.tsx` |
...
| ID | Prompt / Trigger | Routing | Output |
| --- | --- | --- | --- |
| PROMPT-20260502-007 | `继续` | follow-on implementation | Closed ARMATURE-SWARM-021 by consolidating model metadata into a single runtime/catalog source and refreshing release evidence to `1663` tests |
...
| REQ-20260503-002 | UX | Support Claude-compatible `CLAUDE_CODE_NO_FLICKER=1` plus Armature-native `ARMATURE_TUI=fullscreen` / `ARMATURE_NO_FLICKER=1` | Done | `src/ui/render.tsx`, `tests/ink-ui.test.tsx` |
...
| Question | Expected Answer | Guard |
| --- | --- | --- |
| Should Armature enable fullscreen/no-flicker by default? | No. Default remains copyable primary-buffer rendering; no-flicker is explicit opt-in. | `shouldUseNoFlickerRenderer()` |
| Which envs enable no-flicker mode? | `ARMATURE_TUI=fullscreen`, `ARMATURE_NO_FLICKER=1`, `ARMATURE_ALT_SCREEN=1`, or `CLAUDE_CODE_NO_FLICKER=1`. | `tests/ink-ui.test.tsx` |
| Which env disables no-flicker when aliases are also present? | `ARMATURE_TUI=default` and other disabled values (`0`, `false`, `no`, `off`, `scrollback`) win. | `tests/ink-ui.test.tsx` |
| Does no-flicker mode re-enable mouse capture? | No. Mouse capture remains opt-in through `ARMATURE_MOUSE=1`. | `src/ui/components/App.tsx` |
...
| REQ-20260502-016 | DX | Allow root `armature --cwd <dir>` launchers to forward the cwd into chat | Done | `src/program.ts`, `tests/program.test.ts` |
...
| Question | Expected Answer | Guard |
| --- | --- | --- |
| Should Armature default to alternate screen because it uses Ink? | No. Ink can render in the primary buffer; fullscreen/no-flicker alternate screen is opt-in through `ARMATURE_TUI=fullscreen`, `ARMATURE_NO_FLICKER=1`, `ARMATURE_ALT_SCREEN=1`, or `CLAUDE_CODE_NO_FLICKER=1`. | `src/ui/render.tsx`, `tests/ink-ui.test.tsx` |
| Should Armature capture mouse events by default? | No. Mouse capture blocks normal terminal selection; it is opt-in through `ARMATURE_MOUSE=1`. | `src/ui/components/App.tsx` |
| What cwd should tools use when Armature launches from a menu or home directory? | Explicit cwd/env wins; otherwise an ambient project cwd wins; otherwise Armature falls back to the last remembered project workspace. | `src/commands/chat-support.ts` |
...
| ID | Type | Requirement | Status | Evidence |
| --- | --- | --- | --- | --- |
| REQ-20260502-001 | Quality | Add a first-class `armature critique` gate that is distinct from `reflect` and performs read-only reviewer challenge on plans, diffs, tests, and risk assumptions | Done | `src/commands/critique.ts`, `src/critique.ts`, `README.md` |
...
| REQ-20260502-006 | DX | Expose the same critique risk inspection inside `armature chat` through `/critique` without leaving the active session | Done | `src/commands/chat-slash-readonly.ts`, `tests/chat-slash-readonly.test.ts` |
...
| REQ-20260502-008 | Reliability | Keep project hook trust evaluation stable when the singleton is constructed before `ARMATURE_TRUST_PROJECT_HOOKS` is set | Done | `src/hooks.ts`, `tests/v050-modules.test.ts` |
...
| PROMPT-20260502-005 | `继续` | follow-on implementation | Extended automatic local critique hints to one-shot `armature chat "prompt"` without polluting `--json` output |
...
| Does local validation require live provider credentials? | No. `armature critique --dry-run --json` returns checkpoint, risk, reviewer, and diff signals without calling a model. | `src/commands/critique.ts`, `tests/critique.test.ts` |
...
| Can automatic critique hints be controlled without env var changes? | Yes. `armature chat --no-auto-critique` disables them for a session, and `armature chat --auto-critique-threshold <score>` tunes the session threshold. | `src/commands/chat.ts`, `tests/command-contracts.test.ts` |
| Does one-shot `armature chat --json` include automatic critique notices in JSON output? | No. The one-shot helper emits hints only for streaming output and stays silent for JSON output. | `src/commands/chat.ts`, `tests/chat-one-shot-mcp-cleanup.test.ts` |
| Can project hook trust depend only on `HookManager` construction time? | No. `load()` must re-check `ARMATURE_TRUST_PROJECT_HOOKS` so test/runtime paths that set trust before loading hooks remain deterministic. | `src/hooks.ts`, `tests/v050-modules.test.ts` |

## 2026-05-01 - Armature Brand Positioning Correction

### Requirements

| ID | Type | Requirement | Status | Evidence |
| --- | --- | --- | --- | --- |
| REQ-20260501-001 | Brand | Treat Armature's visual identity as killer whale / tiger whale positioning, not generic deep-sea or abstract ocean branding | Done | `ARMATURE_VISUAL_SYSTEM_PLAN.md`, `src/ui/components/Banner.tsx`, `src/ui/components/ThemePicker.tsx` |
| REQ-20260501-002 | Brand | Treat Armature as a killer-whale + ocean joint motif where killer whale is primary, ocean is the field, and pod intelligence is the product metaphor | Done | `ARMATURE_VISUAL_SYSTEM_PLAN.md`, `PDCA_EXECUTION_PLAN.md` |
...
| REQ-20260501-005 | UX | Extend Armature's pod identity into the input, slash-command picker, option picker, and shared picker frame | Done | `src/ui/components/InputArea.tsx`, `src/ui/components/CommandPicker.tsx`, `src/ui/components/OptionPicker.tsx`, `src/ui/components/PickerFrame.tsx` |
| REQ-20260501-006 | UI | Replace hard-coded generic picker colors in changed picker surfaces with Armature semantic theme tokens | Done | `src/ui/components/PickerFrame.tsx`, `src/ui/components/CommandPicker.tsx`, `src/ui/components/OptionPicker.tsx` |
...
| REQ-20260501-010 | UX | Render assistant transcript panels as `ARMATURE POD` while preserving markdown structure | Done | `src/ui/components/App.tsx`, `tests/ink-ui.test.tsx` |
| REQ-20260501-011 | UI | Add Armature scan identity to tool-call rails without hiding tool names, paths, status, or duration | Done | `src/ui/components/App.tsx`, `src/ui/components/ToolCallBlock.tsx`, `tests/ink-ui.test.tsx` |
| REQ-20260501-012 | UX | Replace generic thinking verbs with compact Armature / pod / proof-oriented status copy | Done | `src/ui/components/ThinkingSpinner.tsx`, `tests/ink-ui.test.tsx` |
...
| REQ-20260501-014 | UI | Preserve model, context bar, branch, metrics, policy summaries, permissions, mode, and effort while adding Armature status-rail identity | Done | `src/ui/components/StatusBar.tsx`, `tests/ink-ui.test.tsx` |
...
| PROMPT-20260501-003 | `开始全面优化ui和ux，logo的要学习hermes agent实现可爱的虎鲸形象` | `frontend-design` mascot implementation pass | Implemented cute terminal armature mascot, `POD BRIEF` first-screen UX, docs, and verification |
| PROMPT-20260501-004 | `继续` | `frontend-design` command-surface pass | Extended Armature pod identity into input, command picker, option picker, picker frame, docs, and verification |
| PROMPT-20260501-005 | `继续` | `frontend-design` transcript-flow pass | Extended Armature pod identity into transcript roles, tool rails, thinking state, docs, and verification |
| PROMPT-20260501-006 | `继续` | `frontend-design` status-rail pass | Extended Armature pod identity into the fixed StatusBar with sonar context, signal metrics, trust posture, docs, and verification |
| PROMPT-20260501-007 | `继续` | `frontend-design` proof-wake pass | Extended Armature pod identity into post-turn summaries with `PROOF WAKE`, explicit metrics, docs, and verification |
| PROMPT-20260501-008 | `继续` | `frontend-design` trust-gate pass | Extended Armature pod identity into permission prompts and write diff previews with `TRUST GATE`, `SCAN`, and `ECHO DIFF`, plus docs and verification |
| PROMPT-20260501-009 | `继续` | `frontend-design` evidence-drawer pass | Extended Armature pod identity into detail panels with `EVIDENCE DRAWER`, `pod scan`, theme tone tokens, docs, and verification |
| PROMPT-20260501-010 | `继续` | `frontend-design` council-runway pass | Extended Armature pod identity into multi-model progress with `POD COUNCIL`, `voices`, `surfaced`, `sonar`, theme tokens, docs, and verification |
| PROMPT-20260501-011 | `继续` | `frontend-design` helm-footer pass | Extended Armature pod identity into the persistent footer with `POD HELM`, `interrupt echo`, `send brief`, `pod commands`, width-aware rendering, docs, and verification |

### Anti-Regression Q&A

| Question | Expected Answer | Guard |
| --- | --- | --- |
| Is Armature's brand a generic deep-sea console? | No. Armature is a killer-whale / tiger-whale identity with blackfin, pod, dorsal-fin silhouette, and echolocation cues. | `ARMATURE_VISUAL_SYSTEM_PLAN.md`, `tests/ink-ui.test.tsx` |
| Does ocean imagery belong in the Armature system? | Yes, but as field/environment. The killer whale remains the primary motif and the product metaphor is pod intelligence. | `ARMATURE_VISUAL_SYSTEM_PLAN.md` |
...
| Should command and option pickers keep generic cyan/yellow styling after the Armature refresh? | No. Changed picker surfaces should use Armature semantic theme tokens and pod-language labels. | `src/ui/components/PickerFrame.tsx`, `src/ui/components/CommandPicker.tsx`, `src/ui/components/OptionPicker.tsx` |
...
| Should transcript roles remain generic `You` / `ORCA` after the Armature refresh? | No. User prompts are `POD BRIEF`; assistant responses are `ARMATURE POD`; tools are `ECHO TOOL`. | `src/ui/components/App.tsx`, `src/ui/components/ToolCallBlock.tsx`, `tests/ink-ui.test.tsx` |
...
| Should the fixed status bar read like a generic CLI footer after the Armature refresh? | No. It should keep `ARMATURE POD`, add sonar context language, use `signal:` for stats, and use `trust:` for permissions while preserving all operational fields. | `src/ui/components/StatusBar.tsx`, `tests/ink-ui.test.tsx` |
| Should post-turn summaries use internal shorthand such as `r`, `d`, and `u` after the Armature refresh? | No. Completed turns should render a `PROOF WAKE` summary with explicit time, input, output, tools, cost, and throughput labels. | `src/ui/components/TurnSummary.tsx`, `tests/ink-ui.test.tsx` |
| Should permission prompts remain generic approval panels after the Armature refresh? | No. They should render as `TRUST GATE`, show the tool under review, expose the impact preview under `SCAN`, and keep once/session/project/deny decisions clear. | `src/ui/components/PermissionPrompt.tsx`, `tests/ink-ui.test.tsx` |
...
| Should detail panels render as generic boxes after the Armature refresh? | No. They should render as `EVIDENCE DRAWER` with `pod scan` subtitle context while preserving the source title, subtitle, and markdown body. | `src/ui/components/DetailPanel.tsx`, `tests/ink-ui.test.tsx` |
| Can detail panel tone colors be hard-coded after the Armature refresh? | No. Detail panel info/warn/error borders should resolve through active theme semantic tokens. | `src/ui/components/DetailPanel.tsx` |
| Should multi-model progress remain a generic model list after the Armature refresh? | No. Council, race, and pipeline progress should render as `POD COUNCIL`, with model count as `voices`, completed rows as `surfaced`, and active rows as `sonar`. | `src/ui/components/MultiModelProgress.tsx`, `tests/ink-ui.test.tsx` |
| Can multi-model progress colors be hard-coded after the Armature refresh? | No. Progress header, active, completed, and model-name tones should resolve through active theme semantic tokens. | `src/ui/components/MultiModelProgress.tsx` |
| Should the persistent footer stay as generic shortcut copy after the Armature refresh? | No. It should render as `POD HELM` and use pod-oriented labels while preserving the actual shortcut keys. | `src/ui/components/Footer.tsx`, `tests/ink-ui.test.tsx` |
...
## 2026-04-30 - Armature Visual System / Hermes-Inspired CLI Identity

### Requirements

| ID | Type | Requirement | Status | Evidence |
| --- | --- | --- | --- | --- |
| REQ-20260430-001 | UX | Optimize local Armature CLI by studying Hermes Agent's high-recognition terminal experience | Done | `ARMATURE_VISUAL_SYSTEM_PLAN.md`, `src/ui/components/Banner.tsx` |
| REQ-20260430-002 | Design | Define a distinctive killer-whale Armature visual system covering typography, color, UI, and UX before implementation | Done | `ARMATURE_VISUAL_SYSTEM_PLAN.md`, `ARMATURE_VISUAL_SYSTEM_PLAN.html` |
...
| REQ-20260429-007 | Runtime | Operators need live TaskRun evidence streaming from the queue surface | Done | `armature queue follow`, `tests/queue-command.test.ts` |
...
| REQ-20260429-009 | Runtime | Operators need a bounded lease takeover surface for non-terminal TaskRuns | Done | `armature queue takeover`, `src/work-session-store.ts`, `tests/queue-command.test.ts`, `tests/work-session-store.test.ts` |
...
| REQ-20260429-011 | UX | Operators need a TaskRun evidence drawer for logs, diffs, data, reports, missing artifacts, and bounded previews | Done | `armature queue evidence`, `src/commands/queue.ts`, `tests/queue-command.test.ts` |
...
| REQ-20260429-015 | Runtime | `armature run` default, goal-loop, mission, and plan branches must write canonical WorkSession / TaskRun records | Done | `src/commands/run.ts`, `tests/run-work-session.test.ts` |
...
| REQ-20260429-019 | Runtime | Interactive `armature chat` REPL turns must create and close canonical WorkSession / TaskRun records with status, usage, duration, and runtime evidence | Done | `src/commands/chat.ts`, `src/commands/chat-repl-turn.ts`, `src/work-session-store.ts`, `tests/chat-repl-turn.test.ts`, `tests/work-session-store.test.ts` |
...
| REQ-20260429-023 | Runtime | TaskRun leases must support actionable resume/schedule plans without pretending unsupported run kinds are replay-safe | Done | `armature queue resume`, `armature queue schedule`, `src/commands/queue.ts`, `src/work-session-store.ts`, `tests/queue-command.test.ts` |
...
| PROMPT-20260429-005 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-009: `queue evidence` drawer |
| PROMPT-20260429-006 | `继续` | Continue queued PDCA execution | Core completed ARMATURE-SWARM-010: shared slash-command registry |
| PROMPT-20260429-007 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-011: release evidence snapshot guard |
| PROMPT-20260429-008 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-012: CI gate integrity enforcement |
| PROMPT-20260429-009 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-013: `armature run` execution contract records |
| PROMPT-20260429-010 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-014: clean-index command baseline |
| PROMPT-20260429-011 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-015: chat operator control plane |
| PROMPT-20260429-012 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-016: chat REPL canonical TaskRun records |
| PROMPT-20260429-013 | `继` | Continue queued PDCA execution | Completed ARMATURE-SWARM-017: Ink `/evidence` TaskRun detail panel |
| PROMPT-20260429-014 | `orca发现几个问题...我输入的提示词看不到...内容输出缺乏结构化的表达` | Screenshot-driven Ink UX fix | Completed ARMATURE-SWARM-019: visible prompt blocks and structured assistant response panels |
| PROMPT-20260429-015 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-018: TaskRun approval timeline in CLI / Ink evidence drawer |
| PROMPT-20260429-016 | `继续` | Continue queued PDCA execution | Completed ARMATURE-SWARM-020: `queue resume` and `queue schedule` lease-backed recovery plans |
...
| Is there a CLI surface for existing TaskRun records? | Yes: `armature queue list/show/follow/takeover`. | `tests/queue-command.test.ts` |
| Can operators stream TaskRun evidence without opening raw files? | Yes: `armature queue follow <id>` tails evidence and exits when the run is terminal. | `tests/queue-command.test.ts` |
...
| Can operators inspect TaskRun evidence without opening raw files? | Yes. `armature queue evidence <id>` renders typed evidence entries with paths, metadata, missing-file state, and capped previews. | `tests/queue-command.test.ts` |
...
| Does `armature run` create queue-visible records across its branch modes? | Yes. Default, goal-loop, mission, and plan branches create WorkSession / TaskRun records and finish them with status, summary, usage, and mission evidence when available. | `tests/run-work-session.test.ts` |
...
| Does interactive `armature chat` create queue-visible records for normal prompt turns? | Yes. Each normal REPL prompt is wrapped in a chat `TaskRun` under the REPL `WorkSession`, then finished with status, usage, cost, duration, and runtime observation evidence. | `tests/chat-repl-turn.test.ts`, `tests/work-session-store.test.ts` |
| Can Ink inspect TaskRun evidence without opening raw files? | Yes. `/evidence <task-run-id>` opens the same TaskRun evidence drawer model as `armature queue evidence` in an Ink `DetailPanel`. | `tests/chat-slash-readonly.test.ts`, `tests/queue-command.test.ts` |
...
| Can approval decisions disappear after a review-before-apply prompt? | No. Prompted, preapproved, policy-blocked, and hook-blocked decisions append to `TaskRun.approvals` and render in `armature queue evidence` plus Ink `/evidence`. | `tests/chat-proxy-tool-call.test.ts`, `tests/work-session-store.test.ts`, `tests/queue-command.test.ts` |
...
| REQ-006 | 2026-04-12 | feature | Internalize Hermes-inspired runtime ergonomics into Armature CLI, prioritizing core agent-loop capabilities over gateway/platform-specific features | done | Hermes release mapped to Armature runtime boundaries and implemented in Armature-local runtime seams |
...
| REQ-008 | 2026-04-12 | architecture | Update SDK only if the new capability crosses a reusable provider-neutral runtime seam | done | `MARUCIE-open-agent-sdk` reviewed; no code change required for this Armature-local bundle |
...
| REQ-010 | 2026-04-12 | feature | Reuse the model catalog in `armature providers` and startup output so pre-session inspection matches in-session model selection | done | `src/commands/providers.ts`, `src/commands/chat.ts`, `tests/providers-command.test.ts` |
| REQ-011 | 2026-04-12 | feature | Add Hermes-inspired centralized runtime logging and a lightweight `armature logs` surface | done | `src/logger.ts`, `src/commands/logs.ts`, `tests/logger.test.ts`, `tests/logs-command.test.ts` |
| REQ-012 | 2026-04-12 | feature | Add a doctor-style diagnostics surface so Armature runtime/config health can be inspected without manual file spelunking | done | `src/doctor.ts`, `src/commands/doctor.ts`, `tests/doctor-command.test.ts` |
...
| REQ-022 | 2026-04-14 | ide-integration | Add a real VS Code integration surface so IDE integration is no longer just a roadmap gap | done | `integrations/vscode-armature/` skeleton + `tests/vscode-extension.test.ts` |
| REQ-023 | 2026-04-14 | multimodal | Add a real one-shot multimodal image path so Armature can send local images through proxy providers | done | `openai-compat.ts` prompt parts + `armature chat --image` + multimodal tests |
...
| REQ-040 | 2026-05-29 | code-review | Integrate the large-PR multi-model review workflow into Armature CLI so independent model reports, synthesis, human checkbox decisions, fix logs, review verdicts, and E2E evidence are produced as one durable review ledger | done | `src/review-ledger.ts`, `src/commands/review-ledger.ts`, `tests/review-ledger.test.ts`, `README.md`, `doc/reviews/*` output contract |
| REQ-041 | 2026-05-29 | quality-gate | Reconcile `ai check` with Armature CLI's real project shape so the required project gate no longer expects `tests/test_all.py` or fails legacy docs/no-emoji debt unrelated to the current feature | done | `tests/test_all.py`, docs frontmatter/changelog normalization, no-emoji cleanup, temp git identity setup in tests, `$ARMATURE_HOME`-isolated usage DB tests, `outputs/check/20260529-012813-244bdac2` |
...
| PROMPT-005 | Port GitHub Copilot Rubber Duck into Armature but rename and upgrade it | Make it Armature-native rather than a clone: explicit `reflect` entrypoints, persistent mode, structured diagnosis output, and conservative auto-triggering instead of opaque background duplication | Avoid GitHub/Rubber Duck branding in public UX |
...
| PROMPT-016 | Turn a successful multi-model code review practice into an Armature command | Keep first-pass model reports independent, synthesize only after all reports are saved, preserve human checkboxes as the decision gate, and separate fix agent, review agent, and E2E evidence | Do not let LLM agreement auto-fix issues; agreement raises review priority but Maurice decides accepted/rejected/deferred |

## Anti-Regression Q&A

| Question | Answer |
| --- | --- |
| What is the canonical project doc root? | `doc/00_project/initiative_armature/` |
...
| Which Hermes-inspired capabilities are now internalized in Armature? | Tool arg coercion, oversized tool result persistence, detached background work, provider-aware model inspection, local logs, doctor diagnostics, serve metadata parity, and stats runtime dashboarding. |
| Did the SDK need a matching code change? | No. This bundle is Armature-local runtime ergonomics, not a shared SDK seam yet. |
...
| What does `armature providers` add now? | It shows readiness plus the same context/pricing/caution metadata used by the REPL model catalog. |
| Where do Armature runtime logs live now? | `~/.armature/logs/` or `$ARMATURE_HOME/logs/`, with `agent.log` and `errors.log`. |
| What does `armature doctor` cover? | Provider/config readiness, hooks, MCP, sessions, background jobs, log files, project context, and git availability. |
...
| What IDE integration exists now? | A zero-dependency VS Code extension skeleton can launch Armature chat, current-file review, selection review, MCP server, and doctor directly from VS Code terminals. |
| What multimodal support exists now? | `armature chat --image <path...> "prompt"` works on the proxy path by sending local images as `image_url` content parts; interactive REPL image paste is still not implemented. |
...
| Why was the theme picker reappearing on every launch? | Two issues compounded: the ink app only checked `ARMATURE_THEME` when deciding whether to show onboarding, and the persisted-theme helper used CommonJS `require('fs')` inside an ESM runtime, so `~/.armature/theme` reads fell into the catch path. Launch gating now respects both sources and reads the file with ESM-safe `node:fs`. |
...
| Does choosing a theme only affect the next launch? | No. Theme selection now updates the active ink theme immediately in the current session and is still persisted to `~/.armature/theme` for future launches. |
...
| What concrete regression did the first depth tranche uncover? | `session` used a recursive default action that could stack overflow, and session recovery logic needed to skip corrupted latest files while honoring `ARMATURE_HOME`; both are now covered by `tests/session-command.test.ts` and fixed in the runtime. |
| What do the packaging smokes prove now? | `tests/packaging-smoke.test.ts` proves `npm run build` emits the packaged dist entrypoints, `npm pack --json --dry-run` ships them, and the built `dist/bin/armature.js` binary serves both `--help` and `doctor --json` correctly. |
...
| Where should the large-scale evaluation plan live? | In repo-root `AGENT_EVAL_PLAN.md`, with PDCA details under `doc/00_project/initiative_armature/`. |
...
| What changed in trust hardening tranche 1? | Legacy config `default` now resolves to REPL `auto`, and non-loopback `serve` now requires `ARMATURE_SERVE_TOKEN` plus `Authorization: Bearer <token>` on requests. |
...
| How should Armature expose a Rubber-duck-style workflow? | As `reflect`: explicit `armature reflect`, `/reflect`, and `/mode reflect` surfaces, plus conservative auto-triggering only for clear debugging/explanation prompts. |
| What makes Armature `reflect` more SOTA than a plain rubber-duck alias? | It restructures prompts around symptom/hypothesis/evidence/root-cause/next-step, persists as a behavioral mode, and keeps the auto-trigger visible instead of silently spawning duplicate critique agents. |
| Why did `armature chat --image` fail on GitHub Copilot with `Invalid 'tools'`? | Copilot rejects requests with more than 128 tool definitions. Armature now trims the outgoing tool array to 128 for `api.githubcopilot.com` so multimodal requests keep working even when MCP expands the toolset. |
| Why did `armature chat --image` still fail on `copilot/gpt-5.4` after tool trimming? | `gpt-5.x` on the OpenAI-compatible `/v1/chat/completions` path rejects `function tools + reasoning_effort`. Armature now suppresses `reasoning_effort` for that specific chat-completions combination while keeping effort on tool-free requests. |
| Why could `armature chat --image` still feel hung in a MCP-heavy home directory? | One-shot image requests used to auto-connect all configured MCP servers before sending the screenshot. Armature now skips MCP auto-connect for `--image` one-shot requests so screenshot analysis returns without unrelated MCP startup delays. |
| Why could Armature say something was done when it was not? | The model could emit unsupported completion wording, and Armature previously lacked a generic evidence guard plus had lifecycle gaps: one-shot did not load hooks, `Stop` did not fire after responses, and `SubagentStop` did not fire after delegation. |
...
| Are Copilot-style self-review hooks now usable in Armature? | Yes for the fixed lifecycle: one-shot and REPL run `UserPromptSubmit`, `Stop` fires after model output with `ARMATURE_RESPONSE` / `CLAUDE_RESPONSE`, and `SubagentStop` fires after delegated work. |
...
| Why could Armature still fail to create/open requested Markdown files? | Local file handling was prompt-driven plus best-effort false-save repair. Refusal responses or plain generated Markdown without a false save claim did not force a tool call. `buildPostModelRequiredFileWritePlan()` now writes generated artifacts for explicit save targets, and `buildLocalFileEnforcementNotice()` marks missing file operations incomplete. |
| How should Armature review a very large PR with multiple models? | Use `armature review-ledger --pr <number> --models <csv>` or `--diff-file <patch>`. It writes independent reports, a synthesis ledger, human decisions, fix log, review verdict, and E2E evidence artifacts. |
| Does multi-model agreement mean Armature should automatically fix an issue? | No. Agreement raises priority for human review, but fix agents may only act after `05_human_decisions.md` marks the issue `accepted`. |
| Why did `ai check` fail earlier, and what fixed it? | Earlier runs failed because the generic AI-Fleet project gate needed an Armature-local `tests/test_all.py`, required docs metadata/changelogs, no-emoji cleanup, refreshed release evidence counts, deterministic temp git identity in tests, and isolated usage DB state for parallel test runs. Those blockers are now fixed; the final project gate passed in `outputs/check/20260529-012813-244bdac2`. |
