# Armature SOTA Architecture — Upgrade Roadmap
...
Implementation: fork a child process running Armature's agent loop with restricted tool set.
...
### Goal: Extend Armature from CLI tool to platform agent
...
```mermaid
graph TB
    subgraph "L1: Armature Agent SDK"
...
### SOTA Gap Matrix: Armature vs Competitors

| Capability | Claude Code | Codex | Amp | KiloCode | Factory Droid | **Armature v0.8.0** | Gap |
|---|---|---|---|---|---|---|---|
...
| v0.5.0 Session resume | DONE | `armature chat -c` + auto-save |
...
| v0.7.0 Goal-loop controller | DONE | `armature run --done-when` with regex/command/judge |
...
| v0.8.0 ink TUI migration | DONE | React 18 + ink 5, 11-component tree, ChatSessionEmitter event bus, console interception, CC-parity architecture |
| v0.8.0 CC-parity UI | DONE | borderStyle boxes (input + tool calls), Footer shortcuts, PermissionPrompt, MultiModelProgress, CommandPicker, TurnSummary. Matches CC's 922x flexDirection + 24x borderStyle + 30x inverse patterns |
| v0.8.0 1151 tests | DONE | +28 ink UI tests (11 components, event system, CC-parity layout) |
