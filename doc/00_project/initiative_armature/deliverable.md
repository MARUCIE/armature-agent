---
Title: Armature CLI Deliverable
...
Fix the low-level operator-visible regression where Armature wrote assistant conversation text into a requested Markdown file during false-save repair.
...
- `doc/00_project/initiative_armature/*` active PDCA / evidence updates
...
| Item | Result |
| --- | --- |
| Skills | N/A. Armature-specific local-file repair behavior. |
...
Fix the remaining long-session local file/tool-call failure mode, add the missing canonical tool-call test matrix lane, and settle the startup identity on a clear `ARMATURE-AGENT` wordmark plus state deck without separate icon art.
...
- Replaced the old compact startup mark with a dominant `ARMATURE-AGENT` wordmark and removed the rejected independent Blackfin armature hero/icon block from the existing theme-aware Banner deck.
...
- Rendered Banner inspection -> `ARMATURE-AGENT`, `Armature Agent v0.8.16`, `Blackfin Signal`, clean state rows, and no independent hero art
- `npm run lint` -> pass
- `npm run build` -> pass
...
| Item | Result |
| --- | --- |
| Skills | N/A. This is Armature-specific runtime, test-matrix, and Banner identity work. |
| PDCA four docs | Updated PRD, system architecture, user experience map, and platform optimization plan. |
| Root guidance | N/A. No new cross-task AGENTS / CLAUDE rule. |
...
| --- | --- | --- |
| Already-running Armature sessions use the old built `dist` code until restarted | Operator | Rebuild and restart Armature after this change |
...
Close ARMATURE-SWARM-021 by eliminating duplicated model metadata tables from runtime consumers and making the provider-aware catalog, token budget, provider request defaults, and output summaries share one canonical source.
...
| Item | Result |
| --- | --- |
| Skills | N/A. This is Armature-specific runtime/catalog consolidation, not a reusable cross-project skill. |
| PDCA four docs | Updated PRD, system architecture, user experience map, and platform optimization plan. |
| Root guidance | N/A. No new cross-task AGENTS / CLAUDE rule. |
...
Fix Armature CLI's terminal copyability/flicker regression, launcher cwd drift, MCP server-name routing bug, and missing local file-opening tool.

### Delivered

- Made Ink alternate-screen rendering opt-in through `ARMATURE_ALT_SCREEN=1` so default output stays in normal terminal scrollback and is copyable.
- Made mouse tracking opt-in through `ARMATURE_MOUSE=1` so default text selection is not captured by the app.
...
- Added root `armature --cwd <dir>` forwarding into `armature chat` for launcher/menu entrypoints.
...
| Item | Result |
| --- | --- |
| Skills | N/A. This is Armature-specific runtime hardening, not a reusable cross-project skill. |
| PDCA four docs | Updated PRD, system architecture, user experience map, and platform optimization plan. |
| Root guidance | N/A. No new cross-task AGENTS / CLAUDE rule. |
...
| Risk | Owner | Follow-up |
| --- | --- | --- |
| First launch from a non-workspace directory before any workspace is remembered still uses that directory | Runtime / Launcher | Pass `--cwd`, `ARMATURE_CWD`, or `ARMATURE_PROJECT_DIR` from launch wrappers for deterministic first-run behavior |
...
Optimize Armature CLI against the local 2026-05-02 AI coding CLI research report by adding the missing read-only critique/checkpoint layer while preserving `reflect` as the Socratic debugging workflow.
...
- Added `armature critique` as a first-class command with `--checkpoint`, plan/log/diff inputs, risk flags, `--force`, `--dry-run`, `--json`, and `--show-prompt`.
- Added `/critique` inside `armature chat` as a read-only local inspection that renders reviewer choice, risk score, diff lines, and changed files in legacy output or an Ink detail panel.
...
- Exposed `armature chat --no-auto-critique` and `armature chat --auto-critique-threshold <score>` so automatic hints are discoverable and session-scoped, not env-only.
- Extended the same automatic local hint to one-shot `armature chat "prompt"` streaming runs while keeping `--json` one-shot output clean.
- Made dry-run mode deterministic and API-key-free so local CI and humans can inspect whether a checkpoint would run.
- Registered the command in the public Armature command surface and slash discovery registries.
- Added focused unit, slash, registry, and command contract coverage.
- Fixed hook trust evaluation so project hooks are honored when `ARMATURE_TRUST_PROJECT_HOOKS=1` is set before `HookManager.load()`, even if the singleton was constructed earlier.
...
- `README.md`
- `doc/00_project/initiative_armature/*` PDCA and project fact-source updates
...
- `node dist/bin/armature.js critique --dry-run --json "review current diff"` -> valid JSON
...
| Item | Result |
| --- | --- |
| Skills | N/A. This pass used the existing `optimize` workflow and produced project-specific Armature CLI code, not a reusable cross-project skill. |
...
| Automatic critique hints are local reminders, not live reviewer calls | Product / Runtime | Keep live model critique behind explicit `armature critique` or `/critique` until automatic model calls have an approval and cost policy |
...
Continue the broader Armature UI/UX optimization by carrying the killer-whale pod identity into the persistent shortcut footer.
...
Continue the broader Armature UI/UX optimization by carrying the killer-whale pod intelligence identity into live multi-model progress for council, race, and pipeline execution.
...
Continue the broader Armature UI/UX optimization by carrying the killer-whale pod identity into detail panels used by status, permission, notes, thread, and TaskRun evidence surfaces.
...
Continue the broader Armature UI/UX optimization by carrying the killer-whale pod identity into permission approval and write diff review boundaries.
...
Continue the broader Armature UI/UX optimization by carrying the killer-whale pod identity into the compact post-turn summary.
...
Continue the broader Armature UI/UX optimization by carrying the killer-whale pod identity into the persistent fixed status bar.

### Delivered

- Kept `ARMATURE POD`, model, context bar / percentage, and branch visible on the first status line.
- Added `sonar` context-load language to make context pressure read as an operational Armature signal.
...
Continue the broader Armature UI/UX optimization by carrying the killer-whale pod identity into the live transcript: user prompts, assistant panels, tool rails, and thinking state.

### Delivered

- Reframed submitted prompt transcript blocks from `You` to `POD BRIEF`.
- Reframed assistant response panels from `ORCA` to `ARMATURE POD`.
- Changed streaming assistant state copy to `echoing`.
- Added `ECHO TOOL` identity to active and completed tool-call rails.
- Replaced broad generic thinking verbs with compact Armature / pod / proof-oriented status copy.
- Updated focused Ink UI regression coverage.
- Synced active PDCA docs, visual plan, rolling ledger, notes, and deliverable.
...
Continue the broader Armature UI/UX optimization by carrying the killer-whale pod identity into input, command discovery, option selection, and shared picker framing.

### Delivered

- Made `PickerFrame.tsx` theme-aware so picker chrome uses Armature semantic colors.
...
- Updated `OptionPicker.tsx` to use Armature theme tokens for selection, labels, descriptions, and scroll affordances.
...
## 2026-05-01 - Cute Armature Mascot UI/UX Tranche
...
Start the broader Armature UI/UX optimization by upgrading the terminal first frame: learn Hermes Agent's logo structure while implementing a friendlier pod-brief entry UX. The mascot part was later removed from startup.
...
Correct the visual-system language so Armature is positioned as a killer whale / tiger whale identity rather than a generic deep-sea console.
...
## 2026-04-30 - Armature Visual System PDCA Tranche

### Scope

Optimize the local Armature CLI Ink experience by studying Hermes Agent's high-recognition terminal startup patterns, then shipping a distinct Armature visual system across typography, palette, UI hierarchy, and operator UX.

### Delivered

- Created the canonical visual system plan and Chinese HTML companion: `ARMATURE_VISUAL_SYSTEM_PLAN.md` and `ARMATURE_VISUAL_SYSTEM_PLAN.html`.
- Added `Blackfin Signal` as Armature's semantic default dark theme without adding dependencies.
...
- `src/ui/components/StatusBar.tsx`
- `tests/ink-ui.test.tsx`
- `doc/00_project/initiative_armature/ARMATURE_VISUAL_SYSTEM_PLAN.md`
- `doc/00_project/initiative_armature/ARMATURE_VISUAL_SYSTEM_PLAN.html`
- `doc/00_project/initiative_armature/*` PDCA and project fact-source updates
...
Close ARMATURE-SWARM-020 by turning queue leases into actionable recovery plans for TaskRuns that already carry safe resume metadata.

### Delivered

- Added explicit TaskRun lease-state classification (`none`, `active`, `expired`).
- Added `armature queue resume <task-run-id>` to claim a resume lease and print the concrete recovery command when available.
- Added `armature queue schedule` to pick the next unleased resumable or monitorable TaskRun.
- Chat WorkSessions with saved-session ids now produce `armature chat --cwd ... --continue <saved-session-id>` commands.
- Running background jobs produce `armature queue follow <task-run-id>` monitor commands; unsupported replay exits without claiming a lease.
...
- `package.json`
- `package-lock.json`
- `doc/00_project/initiative_armature/*`
...
Close ARMATURE-SWARM-018 by making review-before-apply decisions durable and visible through the same TaskRun evidence surfaces used by CLI and Ink.
...
- Rendered an `Approval Timeline` above file evidence in `armature queue evidence` and the shared Ink `/evidence` detail panel.
...
| Queue leases needed actionable resume/schedule semantics | Runtime | Resolved by ARMATURE-SWARM-020 for chat saved-session recovery; non-chat replay remains a separate metadata task |
...
| Review-before-apply still lacks an approval timeline over TaskRun evidence | UX | ARMATURE-SWARM-018 |

## 2026-04-29 - Ink TaskRun Evidence Panel PDCA Tranche

### Scope

Close ARMATURE-SWARM-017 by making TaskRun evidence inspectable inside the Ink REPL through `/evidence <task-run-id>`, while reusing the existing queue evidence drawer contract.

### Delivered

- Exported a shared TaskRun evidence drawer model from `src/commands/queue.ts`.
- Kept `armature queue evidence <task-run-id>` and Ink `/evidence <task-run-id>` on the same markdown rendering path.
...
Close ARMATURE-SWARM-016 by making interactive `armature chat` REPL turns write canonical `WorkSession` / `TaskRun` records that the queue spine can inspect.
...
Close ARMATURE-SWARM-015 by making chat REPL and Ink operator controls for sessions, permissions, models, command output, and detail panels clean-index testable.
...
Close ARMATURE-SWARM-014 by making the command surface already declared in `program.ts` build from a clean staged checkout, while preserving the staged config/provider helpers required by the expanded config regression suite.
...
Advance ARMATURE-SWARM-013 by making `armature run` default, goal-loop, mission, and plan paths write the same WorkSession / TaskRun record family already used by queue and `serve /chat`.

### Delivered

- `armature run` creates a `WorkSession` and active `TaskRun` before execution starts.
...
Complete ARMATURE-SWARM-012 by making CI enforce the matrix, security, performance, and fast agent-eval gates that the docs and package scripts advertise.
...
Complete ARMATURE-SWARM-011 by making README and active PDCA verification counts derive from a checked release evidence snapshot instead of loose manual copies.

### Delivered

- Added `doc/00_project/initiative_armature/verification_snapshot.json` as the active release evidence source.
...
- `doc/00_project/initiative_armature/verification_snapshot.json`
- `doc/00_project/initiative_armature/*`
...
| CI gate runtime may need tuning as matrix scope grows | Verification | Keep ARMATURE-SWARM-012 gate job focused on high-signal rows |

## 2026-04-29 - Slash Command Registry PDCA Tranche

### Scope

Advance ARMATURE-SWARM-010 by making slash-command discovery use one registry across REPL completion, Ink command picker, and `/help`, while leaving HomePanel wiring to the existing unstaged UI baseline.
...
Complete ARMATURE-SWARM-009 as a queue-first evidence drawer so operators can inspect TaskRun logs, diffs, data, reports, missing artifacts, and capped previews without opening raw files.
...
Complete ARMATURE-SWARM-008 by making `serve /chat` write the same canonical `WorkSession` / `TaskRun` records used by the CLI run/queue surface.
...
- Clean staged-index checkout attempted at `/tmp/armature-index-008.WnEC1K`; `npm run lint` is blocked by pre-existing uncommitted baseline dependencies (`src/program.ts` imports command/mode surfaces that are present only in the dirty workspace). This tranche intentionally does not stage those broader changes.
...
| TaskRun evidence bundles are still thin for serve requests | UX/runtime | Execute ARMATURE-SWARM-009 evidence drawer / richer evidence attachments |
...
Complete ARMATURE-SWARM-007 by making non-terminal TaskRun records leaseable from the CLI. This is an operator-control marker, not a scheduler or process-resume implementation.

### Delivered

- `armature queue takeover <task-run-id>`
...
| Risk | Owner | Follow-up |
| --- | --- | --- |
| `queue takeover` is a lease marker, not real execution resume | Runtime/architecture | ARMATURE-SWARM-008 now covers `serve /chat`; remaining resume semantics stay under M2/M3 |
...
Continue the SOTA swarm atomic queue by completing ARMATURE-SWARM-006: make TaskRun evidence followable from the CLI without adding scheduler or lease semantics yet.
...
- `node dist/bin/armature.js queue follow <fixture-task-run> --once --lines 1` -> printed fixture evidence tail

### Remaining Risks

| Risk | Owner | Follow-up |
| --- | --- | --- |
| `queue follow` was read-only and could not claim work | Runtime/architecture | Closed by ARMATURE-SWARM-007 `queue takeover`; real resume remains ARMATURE-SWARM-008+ |
...
Audit Armature with a multi-lane SOTA swarm, publish the routed report, convert findings into a milestone plan and atomic queue, then execute the first PDCA tranche against the highest-risk trust and queue gaps.
...
- Trust hardening:
  - repo-local hooks require explicit project trust via `HookManager({ trustProjectHooks: true })` or `ARMATURE_TRUST_PROJECT_HOOKS=1`
...
- Queue visibility:
  - `armature queue`
  - `armature queue list --status <status> --work-session <id> --limit <n>`
...
- `node dist/bin/armature.js --help` -> `queue` command visible
- `node dist/bin/armature.js queue list --limit 3` -> renders TaskRun Queue empty-state successfully
...
| Project hook trust has an env/programmatic switch but no first-class CLI UX | Runtime/security | Add `armature hooks trust/status` or equivalent approval surface |
| `armature queue` is read-only and does not implement leases or takeover | Runtime/architecture | Execute ARMATURE-SWARM-007 |
| CI still under-enforces documented matrix/security/performance/eval gates | Verification | Execute ARMATURE-SWARM-012 |
...
- `npm run build`
- `npm test` -> `1583/1583`
- `node dist/bin/armature.js --version` -> `0.8.0`
...
- Documentation / delivery surfaces:
  - `README.md`
  - `AGENT_EVAL_PLAN.md`
  - `doc/00_project/initiative_armature/{PRD.md,SYSTEM_ARCHITECTURE.md,USER_EXPERIENCE_MAP.md,task_plan.md,notes.md,PDCA_ITERATION_CHECKLIST.md}`
- Stage artifacts:
...
- `node dist/bin/armature.js bench --json` → `outputs/test/2026-04-22-bench.json`
...
## 2026-04-22 — Global Armature Hook Surface（完成）

### Scope

Allow Armature to load a global native hook file from `~/.armature/hooks.json` so operator-level automations can apply across projects without copying `.armature/hooks.json` into every workspace.

### Delivered

- Added global Armature hook loading in:
  - `src/hooks.ts`
...
- Updated docs:
  ...
  - `doc/00_project/initiative_armature/PLATFORM_OPTIMIZATION_PLAN.md`
...
- Armature now loads project-local and operator-global native hook config together.
- This enables AI-Fleet to install a shared Terminal-title `UserPromptSubmit` hook once at `~/.armature/hooks.json` and have direct `armature` sessions inherit it automatically.
...
Add Cloudflare AI Gateway as a first-class Armature aggregator so provider-prefixed SOTA models can run through one OpenAI-compatible endpoint without forking the runtime.
...
- `npm test`
- `npm run build`
- `armature providers` => `cloudflare` now reports `ready` on this machine

### Result

- Armature now has a second serious aggregator lane alongside Poe/OpenRouter.
...
Push Armature's Ink/TUI finite-choice interactions closer to frontier coding CLIs by replacing ad hoc numbered prompts and divergent picker visuals with one shared picker architecture.
...
- Wave 2 approval/trust flow landed:
  - top-level `armature permissions`
...
Port the spirit of Copilot CLI Rubber Duck into Armature as a renamed `reflect` surface with richer prompt shaping, explicit-first UX, and conservative auto-triggering.
...
- Armature now has a named, reusable reflection/debugging surface instead of relying on ad hoc prompts.
- The feature is explicit-first (`armature reflect`, `/reflect`, `/mode reflect`) but can still auto-trigger for clear debugging/explanation asks with visible inline feedback.
...
Read the current SOTA gap / difference docs, then turn Armature's existing seeded `agent-eval` assets into a real reproducible gate system with fast / nightly / release bundles.
...
- Armature now has a real tiered SOTA evaluation system instead of a plan plus one hard-coded fast runner.
...
- Armature’s next tranche is now constrained more tightly than “do Wave 4 continuity”:
  1. trust hardening
...
- `src/commands/serve.ts`
  ...
  - non-loopback `serve` now requires `ARMATURE_SERVE_TOKEN`
...
- reason: this optimization targets Armature's Ink TUI, not a browser frontend
...
Create the canonical planning package for the next Armature CLI quality-expansion program, grounded in the real current baseline and aligned with PDCA plus `agent-eval-system`.
...
- Initialized `agent-eval/` and replaced generic samples with the first Armature-specific fast-gate pack
  - seeded tasks:
    - `fast-root-help`
    - `fast-session-help`
    - `fast-pr-help`
...
- Added the first deterministic depth tranche:
  - `tests/session-command.test.ts`
    ...
    - `ARMATURE_HOME`-aware session lookup
...
- Added packaging / bin-entry release smokes:
  ...
  - built `dist/bin/armature.js` help + doctor-json behavior now has deterministic regression coverage
...
- Armature CLI now has a canonical large-scale testing plan that starts from the real measured baseline and is currently validated at `1276` tests rather than stale flat-doc counts
...
- Theme onboarding fix:
  - `src/ui/theme.tsx`
    - added explicit detection for configured theme preference from `ARMATURE_THEME` or `~/.armature/theme`
  ...
  - `src/ui/components/App.tsx`
    - first-launch theme picker now respects persisted theme preference instead of only checking `ARMATURE_THEME`
    - theme selection now updates the current session before persisting `~/.armature/theme`
- Regression coverage:
  - `tests/ui-utils.test.ts`
    ...
    - persisted `~/.armature/theme` counts as a valid onboarding-suppression signal
  - `tests/ink-ui.test.tsx`
...
Re-audit Armature's ink UI against CC reference sources, separate report drift from real source gaps, and land the highest-value behavior-accuracy fixes with regression coverage.
...
- IDE integration:
  - added `integrations/vscode-armature/`
...
- Multimodal one-shot:
  ...
  - `src/commands/chat.ts`
    - `armature chat --image <path...> "prompt"` builds data-URL image parts from local files
...
- Armature still does not implement CC's broader `ScrollBox.scrollToElement()` + virtualization stack; current fix addresses the real flex-viewport bug, not the whole CC scroll architecture.
...
- Spinner theming and tool-error rendering are still Armature-specific approximations, not a byte-for-byte CC port.

## Scope

Internalize the first Hermes-inspired runtime capability bundle into Armature CLI and keep canonical project docs in sync.

## Delivered

- Root governance files: `AGENTS.md`, `CLAUDE.md`, `CODEX.md`, `GEMINI.md`
- Git hygiene update: `.gitignore` now ignores `.omx/`
- Canonical project doc tree under `doc/00_project/initiative_armature/`
...
- Hermes-inspired runtime bundle:
  ...
  - oversized tool result persistence to `~/.armature/tool-results/`
...
- Model/provider ergonomics bundle:
  ...
  - `armature providers` now shows the same context/pricing/caution metadata before a session starts
- Centralized logging bundle:
  - `src/logger.ts` writes local runtime logs under `~/.armature/logs/` or `$ARMATURE_HOME/logs/`
  - `armature logs` and `armature logs errors` surface recent log entries
...
- Doctor diagnostics bundle:
  ...
  - `armature doctor` and `armature doctor --json` expose that state directly
...
- Serve observability bundle:
  - `armature serve` now reuses doctor/model-catalog metadata in `/health`, `/providers`, and `/doctor`
  - headless clients can inspect the same runtime state surfaces as CLI users
- Stats dashboard bundle:
  - `armature stats` now combines usage/cost, runtime health, and recent error summaries
- SDK boundary:
  - `MARUCIE-open-agent-sdk` reviewed repeatedly but still intentionally unchanged
  - Current Hermes-inspired slices remain Armature-local runtime ergonomics rather than shared provider-neutral SDK seams
- SDK boundary:
  - `MARUCIE-open-agent-sdk` reviewed but not changed; this bundle remains Armature-local for now
...
- Repo verification:
  ...
  - `npm run build`
  - `npm run bench`
  - `node dist/bin/armature.js --help`
  ...
  - `OPENAI_API_KEY=test-openai-key ARMATURE_PROVIDER=openai node dist/bin/armature.js providers`
  - `ARMATURE_HOME=$(mktemp -d) node dist/bin/armature.js logs`
  - `node --experimental-vm-modules node_modules/.bin/vitest run tests/doctor-command.test.ts`
  - `OPENAI_API_KEY=test-openai-key ARMATURE_PROVIDER=openai node dist/bin/armature.js doctor --json`
...
- `armature chat` REPL now accepts arbitrary local image paths directly in the prompt text on the proxy path.
...
- Armature’s largest remaining SOTA gap is no longer workflow labeling or basic trust UX; it is continuity across terminal / web / IDE plus detached execution with visible evidence.
...
- `armature -c <id>` for exact session resume
...
- Armature now has a first durable session-object continuity surface rather than only local autosave files.
...
- Armature now has an explicit layered test matrix with real commands and real evidence, not just an informal “run lint/test/build” habit.
...
- Fixed one-shot hook loading so direct `armature chat "..."` invocations use the same global hook surface as interactive sessions.
- Fixed lifecycle parity for `Stop` and `SubagentStop`.
- `Stop` hooks now receive the assistant response via `ARMATURE_RESPONSE` / `CLAUDE_RESPONSE`, enabling Copilot-style self-review and pre-completion gates to inspect what the model actually said.
...
- Added explicit no-flicker/fullscreen opt-ins: `ARMATURE_TUI=fullscreen`, `ARMATURE_NO_FLICKER=1`, legacy `ARMATURE_ALT_SCREEN=1`, and Claude-compatible `CLAUDE_CODE_NO_FLICKER=1`.
...
- Added render-failure cleanup so Armature exits the alternate screen if Ink throws before the wrapper mounts.
- Limited completed block rendering to the latest `80` blocks in no-flicker mode to reduce repaint work during long sessions.
- Kept mouse capture opt-in through `ARMATURE_MOUSE=1`.

### Verification

- `npm test -- tests/ink-ui.test.tsx` => passed (`80` tests)
- Dist-level no-flicker env smoke => `default=false`, `ARMATURE_TUI=fullscreen=true`, `CLAUDE_CODE_NO_FLICKER=1=true`, `ARMATURE_TUI=default` override=false
...
- Explicit local file save/create/generate prompts now get runtime enforcement:
  - if the model returns Markdown artifact content for a named target file, Armature writes it with `write_file`
  - if the model refuses or no required file tool runs, Armature marks the file request incomplete instead of accepting the response as completion
...
- Added `armature review-ledger` with alias `armature review-swarm`.
...
- Skills: N/A. The reusable workflow is now a first-class Armature CLI command; no separate project skill was created in this tranche.
...
- Technical debt: live provider/`gh pr` execution remains credential-gated; local validation used deterministic dry-run smoke and mocked orchestration tests. The in-scope `ai check` gate debt was closed with `tests/test_all.py`, docs metadata/changelog normalization, no-emoji cleanup, refreshed release evidence, deterministic temp git identity in tests, and `$ARMATURE_HOME`-isolated usage DB tests.
