# Armature Ecosystem SOTA Gap Analysis — Swarm Brainstorming Report

> v0.3.0 baseline | 2026-04-12 | 3-agent parallel research synthesis

## Executive Summary

Armature CLI v0.3.0 has a solid foundation (41 tools, 10 hook events, multi-model council/race/pipeline, 464 tests). But compared to SOTA coding agents (Claude Code, Codex CLI, Amp CLI, KiloCode, Hermes), Armature has **critical gaps in 6 dimensions** that prevent it from being a genuine SOTA-level CLI.

The good news: AI-Fleet already has mature implementations for most of these gaps. The transplant path is clear.

---

## Current State: Armature v0.3.0 (Honest Assessment)
...
2. **Session resume**: Sessions save to disk but `armature -c` (continue) is not wired up
...
**What Armature lacks**: The entire 18-module harness layer that AI-Fleet has built.
...
**What Armature lacks**: Sub-agent spawning with isolated context, git worktree-based agent teams.

| Feature | Claude Code Has | Armature Has | Gap |
|---|---|---|---|
...
**What Armature lacks**: OS-enforced execution isolation.

| Feature | Codex Has | Armature Has | Gap |
|---|---|---|---|
...
**Impact**: Makes Armature safe for untrusted code execution

### Gap 4: Skills System (from AI-Fleet + Hermes)

**What Armature lacks**: A composable skill system with execution modes.

| Feature | AI-Fleet Has | Hermes Has | Armature Has | Gap |
|---|---|---|---|---|
...
**What Armature lacks**: Multi-channel reach beyond CLI.

| Channel | Hermes Has | Armature Has |
|---|---|---|
| CLI | Yes | Yes |
| HTTP server | Yes | Yes (armature serve) |
| Telegram | Yes | No |
| Discord | Yes | No |
| Slack | Yes | No |
...
**Impact**: Extends Armature from CLI tool to platform agent

### Gap 6: Persistent Memory & Learning (from OpenClaw + AI-Fleet)

**What Armature lacks**: Cross-session learning and knowledge compounding.

| Feature | AI-Fleet Has | OpenClaw Has | Armature Has |
|---|---|---|---|
...
**Effort**: ~1000 LOC
**Impact**: Armature learns from its own mistakes across sessions
...
|---|---|---|---|
| **L1: Armature Agent SDK** | Core agent runtime | Tool system + streaming | + Harness + Isolation + Sandbox |
| **L2: Armature CLI** | User-facing CLI | 41 tools, 10 hooks | + Skills engine + Mode system + Agent teams |
| **L3: ArmatureOS** | Platform layer | armature serve (basic) | + Gateway + Webhook + Channel adapters |

---

## Competitive Positioning After Transplant

| Dimension | Current Leader | Armature After Transplant |
|---|---|---|
| Multi-model collaboration | Armature (unique) | Still leads — no competitor has council/race/pipeline |
...
| Testing rigor | Armature (464 tests) | Leads — most tested open-source agent CLI |
...
|---|---|---|
| MCP server hosting | DONE (v0.5.0) | MCPServer class, `armature serve --mcp`, JSON-RPC 2.0 over stdio |
| Mode system (KiloCode-style) | DONE (v0.5.0) | ModeRegistry, 5 builtin modes, custom modes via .armature/modes.json |
| AGENTS.md auto-discovery | DONE (v0.5.0) | discoverGuidance() scans cwd + parents for AGENTS.md/CLAUDE.md/CODEX.md/.armature/rules/ |
...
| Guidance injection in system prompt | DONE (v0.6.0) | discoverGuidance() called in buildSystemPrompt(), auto-injects AGENTS.md/CLAUDE.md/CODEX.md/.armature/rules/ |
...
| Component | Current State | Problem | Fix |
|---|---|---|---|
| `armature run` | `--max-turns` but no criteria | Runs N turns then stops | Add `--done-when` criteria (regex/test/LLM-judge) |
