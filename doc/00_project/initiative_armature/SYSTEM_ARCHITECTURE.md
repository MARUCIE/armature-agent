---
Title: Armature CLI System Architecture
Scope: project architecture
Owner: Maurice
Status: Active
LastUpdated: 2026-05-29
---
# Armature CLI System Architecture
...
1. Command surface:
   ...
   - Registers `armature review-ledger` with alias `armature review-swarm`.
...
3. Provider routing:
   ...
   - Keeps Armature's aggregator-first strategy while allowing explicit model lists for GPT / Composer / Deepseek style review pods.
...
2. False-save repair safety:
   ...
   - Conversational save confirmations without artifact content return `null`, so Armature does not create a polluted `.md` file.
3. Prompt contract:
...
5. Startup wordmark and clean Blackfin deck:
   ...
   - The startup deck now uses a dominant `ARMATURE-AGENT` wordmark instead of the old compact `ORCA` mark.
   - The rejected independent Armature hero/icon art has been removed from the deck.
   - The deck mirrors Hermes' large-wordmark hierarchy and themed status panel while staying inside Armature's active Ink theme tokens.
...
1. Terminal rendering:
   ...
   - `AlternateScreen` is preserved behind explicit fullscreen/no-flicker flags (`ARMATURE_TUI=fullscreen`, `ARMATURE_NO_FLICKER=1`, `ARMATURE_ALT_SCREEN=1`, or `CLAUDE_CODE_NO_FLICKER=1`) for operators who need Claude-style repaint suppression.
...
2. Mouse input:
   ...
   - Mouse tracking can be restored with `ARMATURE_MOUSE=1`.
3. Workspace cwd resolution:
   - `src/commands/chat-support.ts`
   - `src/commands/chat.ts`
   - Explicit `--cwd`, `ARMATURE_CWD`, and `ARMATURE_PROJECT_DIR` are highest priority.
   - Ambient project directories win over remembered state.
   - Non-workspace launch directories fall back to `~/.armature/last-cwd` when available.
4. Launcher/root command surface:
   - `src/program.ts`
   - Root `armature --cwd <dir>` forwards into the delegated `armature chat` command.
5. MCP routing:
   - `src/mcp-client.ts`
...
3. Command surface:
   ...
   - Adds `armature critique [goal...]` with checkpoint, plan, log, diff, risk, reviewer, provider, dry-run, and JSON flags.
...
4. Chat slash surface:
   ...
   - Adds `/critique` as a read-only local inspection inside `armature chat`.
...
5. Automatic chat hint:
   ...
   - One-shot `armature chat "prompt"` uses `emitOneShotAutoCritiqueNotice` before `executeOneShot`; streaming mode writes the hint to stderr, while JSON mode stays silent.
   ...
   - `armature chat --no-auto-critique` and `armature chat --auto-critique-threshold <score>` override the session behavior; env vars remain the lower-level default path.
6. Provider boundary:
...
7. Safety contract:
   ...
   - Automatic chat hints are advisory only and can be disabled with `ARMATURE_AUTO_CRITIQUE=0`.
8. Hook trust stability:
   - `src/hooks.ts`
   - `HookManager.load()` re-checks `ARMATURE_TRUST_PROJECT_HOOKS` so project hook trust is evaluated at hook-loading time, not only singleton construction time.
...
1. Status identity:
   ...
   - Line 1 keeps `ARMATURE POD`, model, context bar / percentage, and branch, and adds a compact `sonar` label for context load on ordinary-width terminals.
2. Signal stats:
   - `src/ui/components/StatusBar.tsx`
...
1. Transcript roles:
   ...
   - Assistant response blocks now label completed and streaming responses as `ARMATURE POD`; streaming state copy reads `echoing`.
2. Tool-call rails:
   - `src/ui/components/App.tsx`
...
3. Thinking state:
   ...
   - The random status verb pool is now a compact Armature-oriented set for listening, echoing, routing, verifying, and surfacing evidence.
   - The rendered state begins with `POD` to keep generation feedback aligned with the transcript identity.
4. Regression coverage:
   - `tests/ink-ui.test.tsx`
   - Tests assert `POD BRIEF`, `ARMATURE POD`, `ECHO TOOL`, and `POD` thinking state while preserving markdown and tool visibility.
...
1. Shared picker frame:
   ...
   - The frame now reads `useTheme()` directly and resolves border/title/subtitle/footer colors from semantic Armature tokens unless a caller overrides the border.
...
1. Startup logo structure:
   - `src/ui/components/Banner.tsx`
   - `ARMATURE_WORDMARK_LINES` has been replaced by the current `ARMATURE-AGENT` display wordmark.
   - The former mascot/hero block is deleted; startup keeps the themed status rows only.
2. First-screen UX:
   - `src/ui/components/HomePanel.tsx`
   - The primary entry panel is now `POD BRIEF`, with copy that tells the operator to give Armature one outcome while the pod handles plan, tools, and evidence.
...
1. Theme source of truth:
   - `src/ui/theme.tsx`
   - `armatureTheme()` defines the `Blackfin Signal` semantic palette.
   - `resolveInitialTheme()` now returns `armature` as the default dark theme while preserving light mode and alternate themes.
2. Startup identity:
   - `src/ui/components/Banner.tsx`
   - Static `ARMATURE-AGENT` block wordmark replaces the low-recognition swimming art and later compact `ORCA` lockup.
...
4. Identity selection and status:
   ...
   - `src/ui/components/StatusBar.tsx`
   - `Blackfin Signal` is listed first and the status line uses `ARMATURE POD` language.
5. Regression coverage:
   - `tests/ink-ui.test.tsx`
...
1. Hook loading is now split by trust boundary:
   ...
   - repo-local `.armature` / `.claude` hooks require explicit project trust
...
2. `TaskRun` is now visible and leaseable through a first-class CLI queue surface:
   ...
   - `armature queue takeover <task-run-id> --holder <name> --ttl <duration>`
   - `armature queue evidence <task-run-id>`
   - `armature queue resume <task-run-id> --holder <name>`
   - `armature queue schedule --holder <name>`
...
- Keep scheduler / resume semantics honest: chat TaskRuns with saved-session metadata can print concrete `armature chat --continue` recovery commands; non-chat replay remains unsupported until replay-safe argv/prompt metadata is persisted.
...
Armature CLI is a TypeScript ESM CLI product composed of a terminal entry layer, command layer, runtime/config layer, provider bridge, and test/verification layer. It is a CLI runtime, so the primary surface is command invocation and streaming terminal output rather than browser routes.

Hook configuration now resolves from both project-local and operator-global Armature surfaces:
- project: `.armature/hooks.json`, `.armature.json`
- global: `~/.armature/hooks.json`
...
| --- | --- | --- |
| Entry | `src/bin/armature.ts` | Shell entry point and signal handling |
...
| IDE integration | `integrations/vscode-armature/` | VS Code extension skeleton that launches `armature` terminal workflows and MCP server directly |
...
| --- | --- | --- |
| `armature` / `armature chat` | `src/commands/chat.ts` | Interactive REPL and one-shot prompting |
| `armature reflect` | `src/commands/chat.ts`, `src/commands/reflect-mode.ts` | Socratic debugging and root-cause investigation surface |
| `armature run` | `src/commands/run.ts` | Agent task execution with canonical WorkSession / TaskRun records for default, goal-loop, mission, and plan paths |
| `armature council` / `armature race` / `armature pipeline` | `src/commands/multi.ts` | Multi-model collaboration flows |
| `armature bench` | `src/commands/bench.ts` | Benchmark and self-evaluation |
| `armature doctor` | `src/commands/doctor.ts` | Local runtime/config diagnostics |
| `armature logs` | `src/commands/logs.ts` | Runtime log viewer |
| `armature providers` | `src/commands/providers.ts` | Provider introspection |
| `armature stats` | `src/commands/stats.ts` | Usage/cost reporting |
| `armature session` | `src/commands/session.ts` | Session lifecycle |
| `armature pr` | `src/commands/pr.ts` | Pull request review workflow |
| `armature serve` | `src/commands/serve.ts` | Headless HTTP + SSE runtime |
| `armature init` | `src/commands/init.ts` | Local configuration bootstrap |

## Route / Page Map

- Web routes: `N/A`
- Primary interaction surface: terminal commands and REPL slash-style workflows
- Reflect/debugging surface: `armature reflect`, `/reflect`, `/mode reflect`, plus conservative prompt-intent auto-triggering in standard chat
- Headless HTTP surface: `armature serve` (see `src/commands/serve.ts`)
...
- Run canonical task entry: `armature run` writes `TaskRun` records for default, goal-loop, mission, and plan branches and stores usage plus mission-state evidence when available.
- Chat canonical task entry: interactive `armature chat` REPL turns write per-prompt `TaskRun` records under the active chat `WorkSession`, including status, usage, duration, and runtime observation ids.
- Queue recovery entry: `armature queue resume` and `armature queue schedule` reuse the same lease path, then render a resume plan. Chat saved sessions become `armature chat --cwd ... --continue <saved-session-id>` commands; running background jobs become `armature queue follow <task-run-id>` monitor commands.
