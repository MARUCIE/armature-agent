---
Title: Armature CLI User Experience Map
Scope: project UX map
Owner: Maurice
Status: Active
LastUpdated: 2026-05-29
---
# Armature CLI User Experience Map
...
| --- | --- | --- | --- |
| Run independent model reviews | Send the same PR / diff to several models without cross-contaminating their first-pass findings | `armature review-ledger --pr <number> --models <csv>` | `src/commands/review-ledger.ts`, `src/review-ledger.ts` |
| Prepare a deterministic review packet | Generate prompts and ledger templates without spending provider tokens | `armature review-ledger --diff-file <patch> --dry-run --json` | `tests/review-ledger.test.ts`, CLI dry-run smoke |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Generate a clean Markdown file | Ask Armature to generate a `.md` document and get only the requested article/document body in the file | `armature chat` / provider proxy save request | `src/commands/local-file-intent.ts`, `tests/local-file-intent.test.ts` |
...
| Avoid unsafe repair when no artifact exists | If a model only says it saved a file but provides no generated body, Armature does not fabricate a polluted file | post-model repair guard | `tests/local-file-intent.test.ts` |

## 2026-05-02 UX Delta - Local File Tool Continuity and Armature Mark

New operator journeys from the long-session tool-call fix, test matrix closure, and startup mark refresh:

| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Create and open a local Markdown file in a long chat | Ask Armature to write and visually open a `.md` file without the model claiming it has no local access after earlier turns | `armature chat` REPL with history | `src/providers/openai-compat.ts`, `src/system-prompt.ts`, `tests/openai-compat-multimodal.test.ts`, `tests/e2e-workflow.test.ts` |
| Recover a falsely claimed saved file | Say the file is missing and ask Armature to open it; Armature reconstructs the claimed file from chat history, writes it, then opens it | `armature chat` follow-up prompt such as `本地没有这个文件，给我打开` | `src/commands/local-file-intent.ts`, `src/commands/chat-repl-turn.ts`, `tests/local-file-intent.test.ts`, `tests/chat-repl-turn.test.ts` |
...
| Recognize Armature immediately on startup | See a dominant `ARMATURE-AGENT` wordmark and clean session deck without separate mascot/icon art | default Ink startup banner | `src/ui/components/Banner.tsx`, `tests/ink-ui.test.tsx` |
| Keep Hermes-style branding mechanics without copying Hermes | Benefit from large wordmark hierarchy, theme-aware deck, and operational status while preserving Armature identity | Banner / HomePanel first frame | `src/ui/components/Banner.tsx`, `src/ui/theme.tsx` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Trust model capacity labels | See the same context/max-output metadata in startup info, model picker, provider inspection, and runtime budget behavior | `armature chat`, `/model`, `/models`, `armature providers` | `src/model-metadata.ts`, `src/model-catalog.ts`, `src/output.ts`, `tests/model-catalog.test.ts` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Copy Armature output normally | Select and copy visible transcript text from the terminal without fighting alternate screen or mouse capture | default `armature chat` Ink UI | `src/ui/render.tsx`, `src/ui/components/App.tsx`, `tests/ink-ui.test.tsx` |
| Opt into no-flicker fullscreen behavior | Suppress terminal repaint flashes with Claude-style alternate-screen rendering only when explicitly desired | `ARMATURE_TUI=fullscreen armature chat`, `ARMATURE_NO_FLICKER=1 armature chat`, or `CLAUDE_CODE_NO_FLICKER=1 armature chat` | `src/ui/render.tsx`, `src/ui/components/AlternateScreen.tsx`, `src/ui/components/App.tsx` |
| Opt into mouse wheel capture | Restore app-level mouse wheel handling only when explicitly desired | `ARMATURE_MOUSE=1 armature chat` | `src/ui/components/App.tsx` |
| Launch from a menu or home directory | Keep tools pointed at the last real project workspace instead of accidental `~` cwd | `armature`, `ai 7`, launcher wrappers | `src/commands/chat-support.ts`, `tests/chat-support.test.ts` |
| Force a project from any launcher | Pass a specific project directory without changing shell cwd | `armature --cwd <project>`, `armature chat --cwd <project>` | `src/program.ts`, `src/commands/chat.ts`, `tests/program.test.ts` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Challenge a plan before implementation | Ask a separate reviewer to find plan gaps before execution starts | `armature critique --checkpoint after_plan --plan-file <file>` | `src/commands/critique.ts`, `tests/critique.test.ts` |
| Review a complex working-tree diff | Gate a multi-file or sensitive implementation before tests or final review | `armature critique --checkpoint after_complex_implementation` | `src/critique.ts`, `src/commands/critique.ts` |
| Inspect risk without spending tokens | See whether a checkpoint would run and why without live model credentials | `armature critique --dry-run --json` | `tests/critique.test.ts` |
...
| Notice risky dirty diffs in one-shot chat | Get the same local reminder before `armature chat "prompt"` starts a one-shot provider run | one-shot chat pre-send notice | `src/commands/chat.ts`, `tests/chat-one-shot-mcp-cleanup.test.ts` |
...
| Tune automatic critique hints per session | Disable or retune local pre-send hints for the current REPL without changing env vars | `armature chat --no-auto-critique`, `armature chat --auto-critique-threshold 0.4` | `src/commands/chat.ts`, `tests/command-contracts.test.ts` |
...
| Recover from repeated failures | Trigger a critique when the main loop keeps making similar fixes | `armature critique --checkpoint stuck_loop --repeated-failure` | `src/critique.ts` |
| Keep reflection and critique distinct | Use `reflect` for Socratic diagnosis and `critique` for read-only reviewer challenge | `armature reflect`, `armature critique` | `README.md`, `src/commands/reflect-mode.ts`, `src/commands/critique.ts` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Read persistent helm guidance | Keep Armature identity visible in the bottom shortcut rail after the banner scrolls away | Footer | `src/ui/components/Footer.tsx`, `tests/ink-ui.test.tsx` |
...
| Preserve trust-mode awareness | Keep `shift+tab` and the active permission mode/source visible while the footer gains Armature language | Active / idle Footer | `src/ui/components/Footer.tsx` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Track a multi-model pod run | See council, race, or pipeline progress as coordinated Armature pod work | MultiModelProgress | `src/ui/components/MultiModelProgress.tsx`, `tests/ink-ui.test.tsx` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Read detail panels as surfaced evidence | Recognize status, permission, notes, thread, and TaskRun detail panels as one Armature evidence family | DetailPanel | `src/ui/components/DetailPanel.tsx`, `tests/ink-ui.test.tsx` |
| Keep source context visible | See the original detail title and subtitle after the Armature frame label is added | DetailPanel title/subtitle | `src/ui/components/DetailPanel.tsx` |
...
| Preserve markdown evidence readability | Keep body markdown rendering identical while the frame becomes Armature-branded | MarkdownText in DetailPanel | `src/ui/components/DetailPanel.tsx`, `src/ui/components/MarkdownText.tsx` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Read persistent pod identity | Keep Armature identity visible after the startup banner scrolls away | Fixed StatusBar line 1 | `src/ui/components/StatusBar.tsx`, `tests/ink-ui.test.tsx` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Read submitted prompts as operator briefs | Keep the user's own prompt visible as an Armature-owned `POD BRIEF` block | Press Enter in Ink chat | `src/ui/components/App.tsx`, `tests/ink-ui.test.tsx` |
| Read assistant output as pod response | See assistant markdown in an `ARMATURE POD` panel with headings and bullets rendered structurally | Assistant response / turn summary | `src/ui/components/App.tsx`, `src/ui/components/MarkdownText.tsx`, `tests/ink-ui.test.tsx` |
| Understand streaming as live echolocation | Read in-progress assistant output as `ARMATURE POD echoing` rather than a generic stream | Assistant streaming | `src/ui/components/App.tsx` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Brief the pod from the prompt | Start typing from an Armature-owned input surface instead of a generic message box | InputArea empty state | `src/ui/components/InputArea.tsx`, `tests/ink-ui.test.tsx` |
...
| Select finite options with Armature hierarchy | Pick modes, models, themes, or actions with the same Armature semantic color language | OptionPicker surfaces | `src/ui/components/OptionPicker.tsx` |
...
## 2026-05-01 UX Delta - Cute Armature Mascot Entry
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Recognize Armature startup identity | See a dominant `ARMATURE-AGENT` wordmark and clean live state deck, not a mascot/icon block | `armature`, `armature chat` startup | `src/ui/components/Banner.tsx`, `tests/ink-ui.test.tsx` |
...
| Brief the pod | Give Armature one outcome in a friendly first panel rather than a generic mission-control prompt | HomePanel `POD BRIEF` | `src/ui/components/HomePanel.tsx` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Recognize Armature immediately | See a distinctive `ORCA` wordmark and killer-whale pod signal motif before reading details | `armature`, `armature chat` startup | `src/ui/components/Banner.tsx`, `tests/ink-ui.test.tsx` |
...
| Track status consistently | Read the status bar as an Armature pod surface rather than a generic status strip | StatusBar | `src/ui/components/StatusBar.tsx` |
...
| Journey | User Need | Entry | Evidence |
| --- | --- | --- | --- |
| Inspect running/completed agent work | See current TaskRun queue without opening raw files | `armature queue list`, `armature queue show <id>` | `src/commands/queue.ts`, `tests/queue-command.test.ts` |
| Follow live TaskRun evidence | Stream appended evidence logs until a TaskRun reaches a terminal state | `armature queue follow <id>` | `src/commands/queue.ts`, `tests/queue-command.test.ts` |
| Open a TaskRun evidence drawer | Inspect logs, diffs, data, reports, missing artifacts, and capped previews by TaskRun | `armature queue evidence <id>` | `src/commands/queue.ts`, `tests/queue-command.test.ts` |
...
| Review approval history | See prompted, preapproved, policy-blocked, and hook-blocked tool decisions before file evidence | `armature queue evidence <id>`, `/evidence <id>` | `src/policy-executor.ts`, `src/work-session-store.ts`, `tests/chat-proxy-tool-call.test.ts` |
...
| Claim operator control of a TaskRun | Mark a non-terminal TaskRun as owned by an operator for a bounded TTL | `armature queue takeover <id> --holder <name> --ttl <duration>` | `src/commands/queue.ts`, `src/work-session-store.ts`, `tests/queue-command.test.ts`, `tests/work-session-store.test.ts` |
| Resume a chat TaskRun | Claim a bounded resume lease and receive the exact saved-session continue command | `armature queue resume <id> --holder <name>` | `src/commands/queue.ts`, `src/work-session-store.ts`, `tests/queue-command.test.ts` |
| Schedule the next recoverable TaskRun | Skip active leases and unsupported replay records, then claim the next resumable or monitorable item | `armature queue schedule --holder <name>` | `src/commands/queue.ts`, `tests/queue-command.test.ts` |
...
| Trust project hooks explicitly | Prevent arbitrary checked-out repos from running startup hook shell commands | `ARMATURE_TRUST_PROJECT_HOOKS=1` or trusted `HookManager` | `src/hooks.ts`, `tests/hooks.test.ts` |
...
Armature CLI is a command-first product. User journeys are structured around terminal entry points, not URLs.

## Journey Map

| Journey | User Intent | Command / Entry | Source |
| --- | --- | --- | --- |
| Onboard | Configure a provider and initialize usage | `armature init`, env vars, config files | `src/commands/init.ts`, `src/config.ts` |
| Explore interactively | Ask questions or operate in a REPL | `armature`, `armature chat`, `armature -c [id]` | `src/bin/armature.ts`, `src/commands/chat.ts`, `src/commands/session.ts` |
| Run operator-wide session automation | Apply personal runtime hooks such as terminal-title sync across all projects | `~/.armature/hooks.json` + `armature` | `src/hooks.ts` |
| Reflect on a bug or confusing code path | Run a focused rubber-duck-style diagnosis pass with explicit and persistent entrypoints | `armature reflect`, `/reflect`, `/mode reflect` | `src/commands/chat.ts`, `src/commands/reflect-mode.ts`, `src/modes/registry.ts` |
| Enter a workflow preset explicitly | Jump straight into code review, debugging, or architecture/planning without first switching `/mode` manually | `armature review`, `armature debug`, `armature architect` | `src/commands/chat.ts`, `src/program.ts`, `src/modes/registry.ts` |
| Analyze local images | Send one or more local screenshots/images with a text instruction through the proxy path | `armature chat --image <path...> "prompt"` | `src/commands/chat.ts`, `src/providers/openai-compat.ts` |
| Diagnose the runtime | Check config/provider/hook/MCP/session/log state before debugging by hand | `armature doctor` | `src/commands/doctor.ts`, `src/doctor.ts` |
...
| Inspect and persist approval mode | Make trust/approval policy explicit instead of relying on hidden defaults or hotkeys; expose live mode, policy source, stored allowlist counts, and legacy-rule maintenance in one operator surface | `armature permissions`, `armature permissions rules`, `armature permissions revoke`, `armature permissions clear`, `armature permissions normalize`, `/permissions`, `/permissions rules`, `/permissions revoke`, `/permissions clear`, `/permissions normalize` | `src/commands/permissions.ts`, `src/commands/chat-slash-mutations.ts`, `src/config.ts` |
| Check providers before starting | Inspect provider readiness and default-model metadata before entering a session | `armature providers`, `armature providers test` | `src/commands/providers.ts`, `src/model-catalog.ts` |
| Inspect runtime logs | Read recent info/warn/error entries without opening files manually | `armature logs`, `armature logs errors` | `src/commands/logs.ts`, `src/logger.ts` |
| Review runtime dashboard | See usage, runtime health, and recent error signals in one place | `armature stats` | `src/commands/stats.ts`, `src/doctor.ts`, `src/logger.ts` |
| Inspect continuity state headlessly | Query the saved-session object set before attaching a richer client | `armature serve` + `GET /sessions|/sessions/latest` | `src/commands/serve.ts`, `src/session-store.ts` |
| Inspect run continuity state headlessly | Query durable run objects created by `armature run` default, goal-loop, mission, and plan paths | `armature serve` + `GET /work-sessions*`, `GET /task-runs*` | `src/commands/run.ts`, `src/commands/serve.ts`, `src/work-session-store.ts`, `tests/run-work-session.test.ts` |
...
| Resume a specific saved session | Continue an exact durable session object instead of only “latest” | `armature -c <id>` | `src/commands/chat.ts`, `src/commands/session.ts` |
| Inspect one saved session headlessly | Read a single durable session object before designing take-over/resume | `armature serve` + `GET /sessions/:id` | `src/commands/serve.ts`, `src/session-store.ts` |
...
| Inspect headless runtime state | Query health/provider/doctor metadata over HTTP before attaching a client | `armature serve` + `GET /health|/providers|/doctor` | `src/commands/serve.ts`, `src/doctor.ts`, `src/model-catalog.ts` |
| Launch from IDE | Start Armature chat/doctor/MCP from VS Code without hand-writing terminal commands | VS Code commands from `integrations/vscode-armature/` | `integrations/vscode-armature/package.json`, `integrations/vscode-armature/extension.js` |
| Execute work | Run a coding or analysis task | `armature run` | `src/commands/run.ts` |
| Compare models | Get multiple opinions or race for speed | `armature council`, `armature race`, `armature pipeline` | `src/commands/multi.ts`, `src/multi-model.ts` |
| Inspect routing | Check configured providers | `armature providers` | `src/commands/providers.ts` |
| Review cost and sessions | Inspect usage history and saved sessions | `armature stats`, `armature session` | `src/commands/stats.ts`, `src/commands/session.ts` |
| Move session artifacts across contexts | fork / export / import / markdown-share / handoff saved sessions for branching and portability, with metadata sidecars for collaboration artifacts | `armature session fork|export|import|markdown|share|handoff` | `src/commands/session.ts`, `src/session-store.ts` |
...
| Review a PR | Pull and review GitHub PRs | `armature pr` | `src/commands/pr.ts` |
| Run headless | Expose the runtime over HTTP/SSE | `armature serve` | `src/commands/serve.ts` |
| Benchmark quality | Measure runtime quality and capability | `armature bench` | `src/commands/bench.ts` |
...
1. User invokes `armature` or `armature chat`.
2. `src/bin/armature.ts` hands off to `src/program.ts`.
...
2. Armature shows the banner plus a dedicated home panel instead of a long flat command list.
...
1. User invokes `armature reflect`, `/reflect ...`, or switches the session with `/mode reflect`.
...
4. For clear debugging or explanation prompts in standard `armature chat`, Armature can conservatively auto-trigger reflect and shows an inline notice when it does.

### 1aa. Workflow Preset Flow

1. User invokes `armature review`, `armature debug`, or `armature architect`.
...
7. When a selected mode is backed by a workflow preset, Armature also applies the preset's default `effort` / `permission mode` values.
...
1. User invokes `armature permissions` or `/permissions`.
2. `armature permissions` shows the current effective approval mode, persisted project/global config values, rule counts, and the user-facing meaning of each mode.
3. Ink `/permissions` opens a detail panel plus a live picker for `yolo` / `auto` / `plan` and save-to-project/global actions.
4. `/permissions set` updates the live REPL policy immediately; `/permissions save` or `armature permissions set --scope ...` persists it to config.
...
9. `armature permissions rules` and `/permissions rules` expose stored session/project/global rule entries rather than only a count.
10. `armature permissions revoke|clear` and `/permissions revoke|clear` let the operator remove one rule or clear a scope instead of editing config files manually; revoke now supports filter-and-pick selection when the exact rule key is not supplied.
...
12. `armature permissions normalize` and `/permissions normalize` rewrite legacy preview-style rules into canonical descriptors instead of leaving them mixed forever.
...
1. User invokes `armature providers` or `armature providers test`.
...
1. User invokes `armature doctor`.
...
The 2026-04-21 swarm audit adds one sharper constraint to these journeys: they do not close until Armature exposes leaseable session/task objects, queue inspection/take-over, review-grade evidence bundles, and a trust model shared across REPL / MCP / serve.
...
2. `armature -c` resumes the latest session, and `armature -c <id>` resumes a specific saved session object by id.
3. `/status` and the live status bar expose that id so the operator can anchor follow-up actions to the same object.
4. `armature serve` exposes `GET /sessions`, `GET /sessions/latest`, and `GET /sessions/:id` so a future web/IDE client can discover and inspect the durable session object set.
...
7. The shipped `WorkSession` / `TaskRun` slice now covers the default `armature run`, goal-loop, mission, plan, `armature chat`, and `serve /chat` paths and is inspectable through queue and serve surfaces.
8. `armature queue resume` and `armature queue schedule` turn saved chat session metadata into concrete continuation commands, while unsupported non-chat replay stays explicit instead of pretending to resume arbitrary runs.
9. If `serve` binds to a non-loopback host, `ARMATURE_SERVE_TOKEN` is now required and HTTP requests must present a bearer token.

### 1i. Stats Dashboard Flow

1. User invokes `armature stats`.
...
3. `armature logs` and `armature logs errors` surface recent entries without requiring direct file inspection.

### 2. Task Execution Flow

1. User invokes `armature run "task"`.
...
1. User invokes `armature session fork`, `armature session export`, `armature session import`, `armature session markdown`, `armature session share`, or `armature session handoff`.
...
3. Armature writes/loads JSON thread artifacts and can create a handoff clone with source metadata.
...
| Banner | Animated armature swimming art + version info |
| HomePanel | Entry-state onboarding shell with one primary action, trust/state summary, quick paths, and failure help |
| ThemePicker | First-launch theme selection that respects persisted preference in `ARMATURE_THEME` or `~/.armature/theme` |
...
|---|---|---|---|
| SS-1 | User starts `armature chat` in REPL mode | Armature opens the normal interactive session | CLI REPL |
| SS-2 | User enters one or more local image paths in the prompt text | Armature detects the image files, including quoted paths and shell-escaped spaces | prompt assembly in `chat-input.ts` |
| SS-3 | User includes text plus two or more screenshots | Armature sends a single multimodal turn with all images attached | proxy-path multimodal request |
| SS-4 | User asks a follow-up question about the same screenshots | Armature retains the multimodal user turn in history on the proxy path | preserved proxy history |
...
| Step | User Action | System Response | Evidence |
| --- | --- | --- | --- |
| CT-1 | User starts `armature chat` in REPL mode | Armature creates a durable chat `WorkSession` with current provider/model/mode metadata | `src/commands/chat.ts` |
| CT-2 | User submits a normal prompt | Armature creates a `kind: "chat"` `TaskRun` titled from the prompt | `src/work-session-store.ts` |
| CT-3 | Provider turn completes, fails, or aborts | Armature finishes the `TaskRun` with status, token usage, cost, duration, and runtime observation evidence | `src/commands/chat-repl-turn.ts` |
...
| Step | User Action | System Response | Evidence |
| --- | --- | --- | --- |
| HD-1 | User reads a long Armature response while the prompt input is focused | PageUp/PageDown and shifted arrow history scrolling remain available | `getHistoryScrollActivationState()` in `src/ui/components/App.tsx` |
...
| HD-3 | User asks Armature to generate/save a Markdown file at a local path | If the model returns generated Markdown body content, runtime writes it with `write_file` | `buildPostModelRequiredFileWritePlan()` |
| HD-4 | Model refuses or omits required file-tool evidence | Armature emits a local-file enforcement notice and does not treat the request as completed | `buildLocalFileEnforcementNotice()` |
