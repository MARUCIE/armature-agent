# Orca CLI SOTA Experience Gap Report

- Initiative: `orca`
- Date: `2026-04-20`
- Scope: terminal/agent experience comparison across `Claude Code`, `Codex`, `Amp`, `Kilo Code`, `OpenCode`, and `GitHub Copilot`
- Goal: identify learnable interaction, feature, and performance deltas that should reshape Orca CLI's roadmap
...
Current Orca strengths before new work:
...
| --- | --- | ---: | --- |
| Orca CLI | `node dist/bin/orca.js --help` | `287.2ms` | local-cli |
...
- Orca already has a strong local startup/help path relative to the installed peers on this machine.
- startup speed is **not** Orca's biggest SOTA gap.
...
- Orca lacks the same density of first-class workflow toggles around sessions, agents, permissions, and environment shaping
- Orca's plugin/agent ecosystem is much thinner
- Orca does not yet expose as many “operator trust” controls on the CLI surface itself
...
- Orca has session persistence but not the same visible fork/resume/cloud/app mental model
- Orca lacks a first-class “apply remote diff locally” or “cloud tasks” story
- Orca's trust/sandbox language is less crisp than Codex's top-level command/flag model
...
- Orca has sessions and threads, but not the same “threads are the core unit of work and collaboration” maturity
- Orca lacks share/handoff/visibility style collaboration concepts
- Orca's permission UX is improving, but Amp exposes policy management more concretely as an administrative workflow
...
- Orca has modes, but the surrounding workflows are thinner
- Orca lacks checkpoint-style recovery/branching ergonomics
- Orca does not yet turn “mode changes” into broader workflow templates
...
- Orca has serve mode, but not a comparably packaged web/headless parity story
- Orca lacks import/export/session portability ergonomics
- Orca's collaboration/hosting story is much thinner
...
- Orca is much more powerful, but it is not always simpler for quick jobs
- Orca still lacks a sharply separated “quick assistant” lane for minimal-friction use

Learnable difference:

- SOTA experience is not only about more commands; it is also about clearer mode separation for novice vs power use

## 5. Gap Matrix: Orca vs SOTA

| Dimension | Current Orca | SOTA reference | Orca gap |
| --- | --- | --- | --- |
...
| Model/provider routing | strong | Orca is competitive here | low |
| Built-in multi-model collaboration | strong lead | most peers weaker | Orca leads |
...
8. **Make Orca's multi-model lead feel first-class in UX**
...
## 8. Recommended Orca PDCA Program
...
- add `fork`, `export`, `import`, `share`, `handoff` for Orca sessions/threads
...
Orca is **already competitive** on:
...
The next meaningful leap is not “more commands.” It is turning Orca from a powerful terminal runtime into a **coherent agent operating surface**.
