...
| M1 Queue visibility | Expose current TaskRun state to operators | Done | `orca queue list/show` shipped with tests |
...
| ORCA-SWARM-006 | P1 | Add `queue follow` for live logs and evidence | Pending |
...
- `npm run lint` -> pass.
- `npm run build` -> pass.
- Final `npm test` -> `1593` tests passed across `86` files.
